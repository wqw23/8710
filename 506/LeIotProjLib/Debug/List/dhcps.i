...
#line 8 "D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\network\\dhcp\\dhcps.h"
...
#line 9 "D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\network\\dhcp\\dhcps.h"
...
#line 10 "D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\network\\dhcp\\dhcps.h"
...
#line 58 "D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\network\\dhcp\\dhcps.h"
...
#line 3 "D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\network\\dhcp\\dhcps.c"
...
#line 4 "D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\network\\dhcp\\dhcps.c"


static uint8_t dhcp_server_state_machine = (5);
...
static SemaphoreHandle_t dhcps_ip_table_semaphore;

static struct netif * dhcps_netif = 0;
...
static void mark_ip_in_table(uint8_t d)
{
	...
	xQueueGenericReceive( ( QueueHandle_t ) ( dhcps_ip_table_semaphore ), 0, ( ( TickType_t ) 0xffffffffUL ), ( ( BaseType_t ) 0 ) );
	if (0 < d && d <= 32) {
		ip_table.ip_range[0] = ((ip_table . ip_range[0]) | (1 << ((d) - 1)));	



	} else if (32 < d && d <= 64) {
	  	ip_table.ip_range[1] = ((ip_table . ip_range[1]) | (1 << (((d - 32)) - 1)));



	} else if (64 < d && d <= 96) {
		ip_table.ip_range[2] = ((ip_table . ip_range[2]) | (1 << (((d - 64)) - 1)));



	} else if (96 < d && d <= 128) {
		ip_table.ip_range[3] = ((ip_table . ip_range[3]) | (1 << (((d - 96)) - 1)));



	} else if(128 < d && d <= 160) {
		ip_table.ip_range[4] = ((ip_table . ip_range[4]) | (1 << ((d) - 1)));	



	} else if (160 < d && d <= 192) {
		ip_table.ip_range[5] = ((ip_table . ip_range[5]) | (1 << (((d - 160)) - 1)));



	} else if (192 < d && d <= 224) {
		ip_table.ip_range[6] = ((ip_table . ip_range[6]) | (1 << (((d - 192)) - 1)));



	} else if (224 < d) {
		ip_table.ip_range[7] = ((ip_table . ip_range[7]) | (1 << (((d - 224)) - 1)));
	...
	xQueueGenericSend( ( QueueHandle_t ) ( dhcps_ip_table_semaphore ), 0, ( ( TickType_t ) 0U ), ( ( BaseType_t ) 0 ) );
	
}

static void save_client_addr(struct ip_addr *client_ip, uint8_t *hwaddr)
{
	uint8_t d = (uint8_t)(((u8_t*)(client_ip))[3]);
	
	xQueueGenericReceive( ( QueueHandle_t ) ( dhcps_ip_table_semaphore ), 0, ( ( TickType_t ) 0xffffffffUL ), ( ( BaseType_t ) 0 ) );
	_memcpy(ip_table . client_mac[d], hwaddr, 6); 
...
static uint8_t check_client_request_ip(struct ip_addr *client_req_ip, uint8_t *hwaddr)
{
	...
	for(i=100;i<=200;i++)
	{
		...
		if(_memcmp(ip_table . client_mac[i], hwaddr, 6) == 0){
...
{
#line 161 "D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\network\\dhcp\\dhcps.c"
}
...
{       
	...
	if(dhcps_addr_pool_set){
		start = (uint8_t)(((u8_t*)(&dhcps_addr_pool_start))[3]);
		end = (uint8_t)(((u8_t*)(&dhcps_addr_pool_end))[3]);
	}else{
		start = 0;
		end = 255;
	}
...
static uint8_t *add_msg_type(uint8_t *msg_option_base_addr, uint8_t message_type)
{
	...
	msg_option_base_addr[0] = (53);
	msg_option_base_addr[1] = (1);
	...
	uint8_t option_code, uint8_t option_length, void *copy_info)
{
	...
	uint8_t *next_option_start_address = 0;
	...
	case (4):
		_memcpy(option_data_base_address, copy_info, (4));
		next_option_start_address = option_data_base_address + 4;
		break;
	case (2):
		_memcpy(option_data_base_address, copy_info, (2));
		next_option_start_address = option_data_base_address + 2;
		break;
	case (1):
		_memcpy(option_data_base_address, copy_info, (1));
...
static int8_t add_offer_options(uint8_t *option_start_address)
{
	...
	int max_addable_option_len = dhcp_message_total_options_lenth - 4 - 3;	

	if(option_start_address == 0)
		goto ERROR;

	
 
	if(temp_option_addr + 6 -option_start_address <= max_addable_option_len) {
		temp_option_addr = fill_one_option_content(option_start_address, (1),
						(4),(void *)&dhcps_local_mask);
	}else{
		goto ERROR;
	}
        ...
        if(temp_option_addr + 6 -option_start_address <= max_addable_option_len) {
		temp_option_addr = fill_one_option_content(temp_option_addr, (3),
						(4), (void *)&dhcps_local_address);
	}else{
		goto ERROR;
	}
	 ...
	 if(temp_option_addr + 6 -option_start_address <= max_addable_option_len) {
	 	temp_option_addr = fill_one_option_content(temp_option_addr, (6),
						(4), (void *)&dhcps_local_address);	
	}else{
		goto ERROR;
	}
	 ...
	 if(temp_option_addr + 6 -option_start_address <= max_addable_option_len) {
		temp_option_addr = fill_one_option_content(temp_option_addr, (51),
						(4), (void *)&dhcp_option_lease_time);
	}else{
		goto ERROR;
	}
	 ...
	 if(temp_option_addr + 6 -option_start_address <= max_addable_option_len) {
		temp_option_addr = fill_one_option_content(temp_option_addr, (54),
						(4), (void *)&dhcps_local_address);
	}else{
		goto ERROR;
	}
	...
	if(temp_option_addr + 6 -option_start_address <= max_addable_option_len) {
		temp_option_addr = fill_one_option_content(temp_option_addr, (28),
						(4), (void *)&dhcps_subnet_broadcast);
	}else{
		goto ERROR;
	}
	...
	if(temp_option_addr + 4 -option_start_address <= max_addable_option_len) {
		temp_option_addr = fill_one_option_content(temp_option_addr, (26),
						(2), (void *) &dhcp_option_interface_mtu);
	}else{
		goto ERROR;
	}
	...
	if(temp_option_addr + 3 -option_start_address <= max_addable_option_len) {
		temp_option_addr = fill_one_option_content(temp_option_addr, (31),
						(1),	0);
	}else{
		goto ERROR;
	}
...
static void dhcps_initialize_message(struct dhcp_msg *dhcp_message_repository)
{
        ...
        dhcp_message_repository->op = (2);
        dhcp_message_repository->htype = (1);
        dhcp_message_repository->hlen = (6); 
        dhcp_message_repository->hops = 0;		
        _memcpy((char *)dhcp_recorded_xid, (char *) dhcp_message_repository->xid, sizeof(dhcp_message_repository->xid));

        dhcp_message_repository->secs = 0;
        dhcp_message_repository->flags = lwip_htons((0x8000));         

	_memcpy((char *)dhcp_message_repository->yiaddr, (char *)&dhcps_allocated_client_address, sizeof(dhcp_message_repository->yiaddr));


        
	_memset((char *)dhcp_message_repository->ciaddr, 0, sizeof(dhcp_message_repository->ciaddr));

        _memset((char *)dhcp_message_repository->siaddr, 0, sizeof(dhcp_message_repository->siaddr));

        _memset((char *)dhcp_message_repository->giaddr, 0, sizeof(dhcp_message_repository->giaddr));

        _memset((char *)dhcp_message_repository->sname, 0, sizeof(dhcp_message_repository->sname));

        _memset((char *)dhcp_message_repository->file, 0, sizeof(dhcp_message_repository->file));

        _memset((char *)dhcp_message_repository->options, 0, dhcp_message_total_options_lenth);

        _memcpy((char *)dhcp_message_repository->options, (char *)dhcp_magic_cookie, sizeof(dhcp_magic_cookie));
...
{
	uint8_t temp_ip = 0;
	struct pbuf *newly_malloc_packet_buffer = 0;

	
	newly_malloc_packet_buffer = pbuf_alloc(PBUF_TRANSPORT, 236 + 312, PBUF_RAM);
	if(newly_malloc_packet_buffer == 0)
	{
		rtl_printf("\r\n[%s] error:  pbuf alloc fail !", __func__);
		return;
	}
	if(pbuf_copy(newly_malloc_packet_buffer, packet_buffer) != 0)
	{
		rtl_printf("\r\n[%s] error:  pbuf copy fail !", __func__);
		pbuf_free(newly_malloc_packet_buffer);	
		return;
	}	
	dhcp_message_total_options_lenth = 312;
	...
	rtl_printf("\n\r[%d]DHCP assign ip = %d.%d.%d.%d\n", xTaskGetTickCount(), (((u8_t*)(&dhcps_network_id))[0]),(((u8_t*)(&dhcps_network_id))[1]),(((u8_t*)(&dhcps_network_id))[2]),temp_ip);
	(&dhcps_allocated_client_address)->addr = ((u32_t)((temp_ip) & 0xff) << 24) | ((u32_t)(((((u8_t*)(&dhcps_network_id))[2])) & 0xff) << 16) | ((u32_t)(((((u8_t*)(&dhcps_network_id))[1])) & 0xff) << 8) | (u32_t)((((((u8_t*)(&dhcps_network_id))[0]))) & 0xff);


	dhcps_initialize_message(dhcp_message_repository);
	if(add_offer_options(add_msg_type(&dhcp_message_repository->options[4], (2))) == 0)
		udp_sendto_if(dhcps_pcb, newly_malloc_packet_buffer, &dhcps_send_broadcast_address, (68), dhcps_netif);
...
static void dhcps_send_nak(struct pbuf *packet_buffer)
{
	...
	dhcps_initialize_message(dhcp_message_repository);
	add_msg_type(&dhcp_message_repository->options[4], (6));
	udp_sendto_if(dhcps_pcb, packet_buffer,
		   &dhcps_send_broadcast_address, (68), dhcps_netif);
}
...
static void dhcps_send_ack(struct pbuf *packet_buffer)
{
	dhcp_message_repository = (struct dhcp_msg *)packet_buffer->payload;
	dhcps_initialize_message(dhcp_message_repository);
	if(add_offer_options(add_msg_type(&dhcp_message_repository->options[4], (5))) == 0)
		udp_sendto_if(dhcps_pcb, packet_buffer, &dhcps_send_broadcast_address, (68), dhcps_netif);
}
...
uint8_t dhcps_handle_state_machine_change(uint8_t option_message_type)
{
	...
	case (4):
		...
		dhcp_server_state_machine = (5);
		break;
	case (1):



		if (dhcp_server_state_machine == (5)) {
			dhcp_server_state_machine = (1);
		}
		break;
	case (3):
#line 503 "D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\network\\dhcp\\dhcps.c"
		if (dhcp_server_state_machine == (1)) {
			if ((((u8_t*)(&dhcps_allocated_client_address))[3]) != 0) { 
				if (_memcmp((void *)&dhcps_allocated_client_address, (void *)&client_request_ip, 4) == 0) {  	
					dhcp_server_state_machine = (3);
			  	} else {
				  	dhcp_server_state_machine = (4);
			  	}
			} else {
			  	dhcp_server_state_machine = (4);
			}  

		} else if(dhcp_server_state_machine == (5)){
			uint8_t ip_addr4 = check_client_request_ip(&client_request_ip, client_addr);
			if(ip_addr4 > 0){
				(&dhcps_allocated_client_address)->addr = ((u32_t)((ip_addr4) & 0xff) << 24) | ((u32_t)(((((u8_t*)(&dhcps_network_id))[2])) & 0xff) << 16) | ((u32_t)(((((u8_t*)(&dhcps_network_id))[1])) & 0xff) << 8) | (u32_t)((((((u8_t*)(&dhcps_network_id))[0]))) & 0xff);
...
#line 533 "D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\network\\dhcp\\dhcps.c"
		break;
	case (7):
		rtl_printf("get message DHCP_MESSAGE_TYPE_RELEASE\n");
		dhcp_server_state_machine = (5);
		break;
	}
...
#line 569 "D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\network\\dhcp\\dhcps.c"
			_memcpy((char *)&client_request_ip, (char *)option_start + 2, 4);	
...
static uint8_t dhcps_check_msg_and_handle_options(struct pbuf *packet_buffer)
{
	...
	_memcpy(client_addr, dhcp_message_repository->chaddr, 6);
	 
	if (_memcmp((char *)dhcp_message_repository->options, (char *)dhcp_magic_cookie, sizeof(dhcp_magic_cookie)) == 0) {
...
{	
	...
	if (sender_port == (68)) {
		...
		case (3):
			...
			mark_ip_in_table((uint8_t)(((u8_t*)(&dhcps_allocated_client_address))[3])); 			

			save_client_addr(&dhcps_allocated_client_address, client_addr);
			_memset(&client_request_ip, 0, sizeof(client_request_ip));
			_memset(&client_addr, 0, sizeof(client_addr));
			_memset(&dhcps_allocated_client_address, 0, sizeof(dhcps_allocated_client_address));
...
void dhcps_set_addr_pool(int addr_pool_set, struct ip_addr * addr_pool_start, struct ip_addr *addr_pool_end)
{
	...
	if(addr_pool_set){
		...
		_memcpy(&dhcps_addr_pool_start, addr_pool_start, sizeof(struct ip_addr));

		
		
		_memcpy(&dhcps_addr_pool_end, addr_pool_end, sizeof(struct ip_addr));
...
{	
	...
	_memset(&ip_table, 0, sizeof(struct table));
	...
	(&dhcps_send_broadcast_address)->addr = ((u32_t)((255) & 0xff) << 24) | ((u32_t)((255) & 0xff) << 16) | ((u32_t)((255) & 0xff) << 8) | (u32_t)((255) & 0xff);
	 

	_memcpy(&dhcps_local_address, &pnetif->ip_addr, sizeof(struct ip_addr));

	_memcpy(&dhcps_local_mask, &pnetif->netmask, sizeof(struct ip_addr));


	_memcpy(&dhcps_local_gateway, &pnetif->gw, sizeof(struct ip_addr));
	...
	dhcps_owned_first_ip.addr = lwip_htonl((lwip_ntohl(dhcps_network_id . addr) + 1));
	dhcps_owned_last_ip.addr = lwip_htonl(lwip_ntohl(dhcps_subnet_broadcast . addr) - 1);
	dhcps_num_of_available_ips = ((lwip_ntohl(dhcps_owned_last_ip . addr) 
				- lwip_ntohl(dhcps_owned_first_ip . addr)) + 1); 
	...
	if (dhcps_ip_table_semaphore != 0) {	
		vQueueDelete( ( QueueHandle_t ) ( dhcps_ip_table_semaphore ) );
		dhcps_ip_table_semaphore = 0;
	}
	dhcps_ip_table_semaphore = xQueueCreateMutex( ( ( uint8_t ) 1U ) );

	
	_memset(&ip_table, 0, sizeof(struct table));
	mark_ip_in_table((uint8_t)(((u8_t*)(&dhcps_local_address))[3]));
	mark_ip_in_table((uint8_t)(((u8_t*)(&dhcps_local_gateway))[3]));
#line 784 "D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\network\\dhcp\\dhcps.c"
	if(dhcps_addr_pool_start.addr== 0 && dhcps_addr_pool_end.addr == 0)
	{		
		_memcpy(&dhcps_pool_start, &dhcps_local_address, sizeof(struct ip_addr));
		ip = (uint8_t *)&dhcps_pool_start;
		ip[3] = 100;
		_memcpy(&dhcps_pool_end, &dhcps_local_address, sizeof(struct ip_addr));
		ip = (uint8_t *)&dhcps_pool_end;
		ip[3] = 200;
		dhcps_set_addr_pool(1,&dhcps_pool_start,&dhcps_pool_end);
	}
	udp_bind(dhcps_pcb, ((ip_addr_t *)&ip_addr_any), (67));
	udp_recv(dhcps_pcb, dhcps_receive_udp_packet_handler, 0);
}
