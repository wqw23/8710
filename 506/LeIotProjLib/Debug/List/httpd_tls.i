...
#line 26 "D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\network\\httpd\\httpd.h"
...
#line 1 "D:\\vm-share\\506\\LeIotProjLib\\..\\rtk8710_sdk\\component\\common\\network\\ssl\\polarssl-1.3.8\\include\\polarssl/base64.h"
...
static x509_crt httpd_certs;          
static pk_context httpd_key;          
...
int httpd_tls_setup_init(const char *server_cert, const char *server_key, const char *ca_certs)
{
	...
	_memset(&httpd_certs, 0, sizeof(x509_crt));
	_memset(&httpd_key, 0, sizeof(pk_context));
	x509_crt_init(&httpd_certs);
	pk_init(&httpd_key);

	
	if((ret = x509_crt_parse(&httpd_certs, (const unsigned char *) server_cert, prvStrLen((const unsigned char *) server_cert))) != 0) {
	...
	if((ret = x509_crt_parse(&httpd_certs, (const unsigned char *) ca_certs, prvStrLen((const unsigned char *) ca_certs))) != 0) {
		rtl_printf("\n[HTTPD] ERROR: x509_crt_parse %d\n", ret);
		ret = -1;
		goto exit;
	}

	if((ret = pk_parse_key(&httpd_key, (const unsigned char *) server_key, prvStrLen((const unsigned char *) server_key), 0, 0)) != 0) {
...
void *httpd_tls_new_handshake(int *sock, uint8_t secure)
{
	...
	int ret = 0;
	struct httpd_tls *tls = 0;
	ssl_context *ssl = 0;

	if((tls = (struct httpd_tls *) pvPortMalloc(sizeof(struct httpd_tls))) != 0) {
		_memset(tls, 0, sizeof(struct httpd_tls));
		ssl = &tls->ctx;
		...
		ssl_set_ca_chain(ssl, httpd_certs.next, 0, 0);

		if(secure == 2) {
			ssl_set_authmode(ssl, 2);
			ssl_set_verify(ssl, _verify_func, 0);
		}
...
exit:
	...
	return (void *) tls;
#line 301 "D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\network\\httpd\\httpd_tls.c"
}
...
int httpd_base64_encode(uint8_t *data, size_t data_len, char *base64_buf, size_t buf_len)
{
	...
	return ret;
#line 372 "D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\network\\httpd\\httpd_tls.c"
}
