...
#line 33 "D:\\vm-share\\506\\LeIotProjLib\\..\\rtk8710_sdk\\component\\common\\network\\lwip\\lwip_v1.4.1\\src\\include\\lwip/netdb.h"
...
lwip_gethostbyname(const char *name)
{
  ...
  if (err != 0) {
    do { if ( ((0x00U) & 0x80U) && ((0x00U) & 0x80U) && ((s16_t)((0x00U) & 0x03) >= 0x00)) { rtl_printf ("lwip_gethostbyname(%s) failed, err=%d\n", name, err); if ((0x00U) & 0x08U) { while(1); } } } while(0);
    h_errno = 210;
    return 0;
  }
                ...
                size_t buflen, struct hostent **result, int *h_errnop)
{
  ...
  *result = 0;
  ...
  namelen = prvStrLen((const unsigned char *) name);
       ...
       const struct addrinfo *hints, struct addrinfo **res)
{
  ...
  *res = 0;
  ...
  if (nodename != 0) {
    namelen = prvStrLen((const unsigned char *) nodename);
    do { if(!((namelen + 1) <= (mem_size_t)-1)) ; } while(0);
    total_size += namelen + 1;
  }
   
  do { if(!(total_size <= (32 + 16 + 256 + 1))) ; } while(0);
  ...
  if (nodename != 0) {
    ...
    _memcpy(ai->ai_canonname, nodename, namelen);
