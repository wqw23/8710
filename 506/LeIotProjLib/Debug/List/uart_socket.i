#line 1 "D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\utilities\\uart_socket.c"
...
#line 2 "D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\utilities\\uart_socket.c"
#line 1 "D:\\vm-share\\506\\LeIotProjLib\\..\\rtk8710_sdk\\component\\common\\mbed\\targets\\hal\\rtl8711b\\PinNames.h"
...
#line 6 "D:\\vm-share\\506\\LeIotProjLib\\..\\rtk8710_sdk\\component\\common\\mbed\\targets\\hal\\rtl8711b\\PinNames.h"
...
#line 3 "D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\utilities\\uart_socket.c"
...
#line 4 "D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\utilities\\uart_socket.c"
#line 1 "D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\utilities\\uart_socket.h"
...
#line 6 "D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\utilities\\uart_socket.h"
...
#line 8 "D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\utilities\\uart_socket.h"
...
#line 9 "D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\utilities\\uart_socket.h"
...
#line 5 "D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\utilities\\uart_socket.c"
...
#line 6 "D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\utilities\\uart_socket.c"



 
#line 19 "D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\utilities\\uart_socket.c"

#line 27 "D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\utilities\\uart_socket.c"
...
static void uart_irq(uint32_t id, SerialIrq event)
{
	...
	if(event == RxIrq) {
		if( u->rx_start == 0 ){
			rtw_up_sema_from_isr(&u->action_sema);	
			u->rx_start = 1; 
		}
		u->recv_buf[u->prxwrite++] = serial_getc(&u->sobj);
		if(u->prxwrite > (1024 -1)){	
			u->prxwrite = 0;
			u->rxoverlap = 1;							
		}
		if(u->rxoverlap && (u->prxwrite + 1) > u->prxread ){
			u->prxread = u->prxwrite;		
		}
		u->last_update =  xTaskGetTickCountFromISR();	
	}
...
static int uart_send_stream(uart_socket_t *u, char* pbuf, int len)
{
	...
	if(!len || (!pbuf) || !u){
		rtl_printf("input error,size should not be null\r\n");
		return -1;
	}


	while(rtw_down_sema(&u->dma_tx_sema) == ( ( BaseType_t ) 1 )){			
...
#line 97 "D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\utilities\\uart_socket.c"

	return 0;
}

static int32_t uart_wait_rx_complete(uart_socket_t *u)
{
	int32_t tick_current = xTaskGetTickCount();

	while((tick_current -u->last_update) < 20 ){
		vTaskDelay(5);
...
static void uart_action_handler(void* param)
{
	...
	while(rtw_down_sema(&u->action_sema) == ( ( BaseType_t ) 1 )) {
		if(u->fd == -1)
			goto Exit;
		if(u->rx_start){
			...
			lwip_selectevindicate(u->fd);	
			u->rx_start = 0;
		}
		if(u->tx_start){

			if (u->tx_bytes < 128) {
				do{ int i; rtl_printf("\n%s: Len=%d\n", ("TX:"), (u->tx_bytes)); for(i = 0; i < (u->tx_bytes); i++) rtl_printf("%02x ", (u->send_buf)[i]); rtl_printf("\n");}while(0);;
			} else {
				rtl_printf("\nTX:: Len=%d\n", u->tx_bytes);
			}
			...
			if(uart_send_stream(u, (char*)u->send_buf, u->tx_bytes) == -1){
				rtl_printf("uart send data error!");
			} else {
...
uart_socket_t* uart_open(uart_set_str *puartpara)
{
	PinName uart_tx = PA_23;
	PinName uart_rx = PA_18;
	uart_socket_t *u;

	u = (uart_socket_t *)rtw_zmalloc(sizeof(uart_socket_t));
	if(!u){
		rtl_printf("%s(): Alloc memory for uart_socket failed!\n", __func__);
		return 0;
	}
	...
	if(u->fd == -1){
		rtl_printf("Failed to alloc uart socket!\n");
		goto Exit2;
	}
	 ...
	 
	{
		...
		if(xTaskGenericCreate( ( uart_action_handler ), ( ((const char*)"uart_action") ), ( 256 ), ( u ), ( 1 ), ( 0 ), ( 0 ), ( 0 ) ) != ( ( ( BaseType_t ) 1 ) )){
			rtl_printf("%s xTaskCreate(uart_action) failed", __FUNCTION__);
			goto Exit1;
		}
	}
	return u;
...
Exit2:
	rtw_mfree((uint8_t*)u, sizeof(uart_socket_t));
	return 0;
}
...
{
	if(!u){
		rtl_printf("uart_close(): u is NULL!\r\n");
		return -1;
	}
	 
	if(lwip_close(u->fd) == -1){
		rtl_printf("%s(): close uart failed!", __func__);
	}
...
int uart_read(uart_socket_t *u, void *read_buf, size_t size)
{
	...
	rtl_printf("==>uart_read()\n");
	if(!size || !read_buf || !u){
		rtl_printf("uart_read(): input error,size should not be null\r\n");
		return -1;
	}
	...
	if(!u->rxoverlap){
		...
		read_bytes = (1024 - pread_local) + pwrite_local;
	}
	...
	if(!u->rxoverlap){
		_memcpy(ptr, (u->recv_buf+ pread_local), read_bytes);
	} else {
		rtl_printf("uart recv buf is write through!!\n");
		if((pread_local + read_bytes) > 1024){
			_memcpy(ptr, (u->recv_buf+ pread_local), (1024-pread_local));
			_memcpy(ptr+(1024-pread_local), u->recv_buf, read_bytes-(1024- pread_local));
		} else
			_memcpy(ptr, (u->recv_buf+ pread_local), read_bytes);
	}
	...
	if((pread_local + read_bytes) >= 1024){		
		u->prxread = (pread_local + read_bytes) - 1024;
		u->rxoverlap = 0;		
	} else
...
{
	if(!size || !pbuf || !u){
		rtl_printf("input error,please check!");
		return -1;
	}
	if(rtw_down_sema(&u->tx_sema)){
		
		_memcpy(u->send_buf, pbuf, size);
		u->tx_bytes = size;
		u->tx_start = 1;	
		rtw_up_sema(&u->action_sema);	
	} else {
		rtl_printf("uart write buf error!");
		return -1;
	}
...
void uart_socket_example(void *param)
{
	...
	uartset.parity = 0;
	_strcpy(uartset . UartName, "uart0");

	uart_socket = uart_open(&uartset);
	if(uart_socket == 0){
		rtl_printf("Init uart socket failed!\n");
		goto Exit;
	}
	...
	while(1)
	{
		...
		((&readfds)->fd_bits[(uart_fd)/8] |= (1 << ((uart_fd) & 7)));
		tv.tv_sec = 0;
		tv.tv_usec = 20000;
		if(count++ == 50){
		...
		ret = lwip_select(uart_fd + 1,&readfds,0,0,&tv);
		
		if(ret > 0)
		{
			if(((&readfds)->fd_bits[(uart_fd)/8] & (1 << ((uart_fd) & 7))))
			{
				read_len = uart_read(uart_socket, rxbuf, sizeof(rxbuf));
				if(read_len > 0)
				{
					do{ int i; rtl_printf("\n%s: Len=%d\n", ("RX:"), (read_len)); for(i = 0; i < (read_len); i++) rtl_printf("%02x ", (rxbuf)[i]); rtl_printf("\n");}while(0);;
	...
	rtl_printf("Exit uart socket example!\n");
...
void uart_socket()
{
	...
	if(xTaskGenericCreate( ( uart_socket_example ), ( "uart_socket" ), ( 512 ), ( 0 ), ( 1 ), ( 0 ), ( 0 ), ( 0 ) ) != ( ( ( BaseType_t ) 1 ) ))
		rtl_printf("%s xTaskCreate failed", __FUNCTION__);
}
