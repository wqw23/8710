#line 1 "D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\example\\socket_select\\example_socket_select.c"
...
#line 2 "D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\example\\socket_select\\example_socket_select.c"
...
#line 3 "D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\example\\socket_select\\example_socket_select.c"
...
#line 4 "D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\example\\socket_select\\example_socket_select.c"
...
#line 5 "D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\example\\socket_select\\example_socket_select.c"



#line 16 "D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\example\\socket_select\\example_socket_select.c"
...
static void example_socket_select_thread(void *param)
{
	...
	_memset(socket_used, 0, sizeof(socket_used));

#line 59 "D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\example\\socket_select\\example_socket_select.c"
	if((server_fd = lwip_socket(2,1,0)) >= 0) {
		...
		if(lwip_bind(server_fd,(struct sockaddr *) &server_addr,sizeof(server_addr)) != 0) {
			rtl_printf("bind error\n");
			goto exit;
		}

		if(lwip_listen(server_fd,2) != 0) {
			rtl_printf("listen error\n");
			goto exit;
		}
	...
	while(1) {
		...
		_memset((void*)(&read_fds), 0, sizeof(*(&read_fds)));
		timeout.tv_sec = 10;
		timeout.tv_usec = 0;

		for(socket_fd = 0; socket_fd < 10; socket_fd ++)
			if(socket_used[socket_fd])
				((&read_fds)->fd_bits[(socket_fd)/8] |= (1 << ((socket_fd) & 7)));

		if(lwip_select(max_socket_fd + 1,&read_fds,0,0,&timeout)) {
			for(socket_fd = 0; socket_fd < 10; socket_fd ++) {
				if(socket_used[socket_fd] && ((&read_fds)->fd_bits[(socket_fd)/8] & (1 << ((socket_fd) & 7)))) {
					if(socket_fd == server_fd) {
						struct sockaddr_in client_addr;
						unsigned int client_addr_size = sizeof(client_addr);
						int fd = lwip_accept(server_fd,(struct sockaddr *) &client_addr,&client_addr_size);

						if(fd >= 0) {
							rtl_printf("accept socket fd(%d)\n", fd);
							socket_used[fd] = 1;
							...
							rtl_printf("accept error\n");
						}
					}
					else {
						int read_size = lwip_read(socket_fd,buf,sizeof(buf));

						if(read_size > 0) {
							lwip_write(socket_fd,buf,read_size);
						}
						else {
							rtl_printf("socket fd(%d) disconnected\n", socket_fd);
			...
			rtl_printf("TCP server: no data in %d seconds\n", 10);
		}
...
{
	if(xTaskGenericCreate( ( example_socket_select_thread ), ( ((const char*)"example_socket_select_thread") ), ( 1024 ), ( 0 ), ( ( ( UBaseType_t ) 0U ) + 1 ), ( 0 ), ( 0 ), ( 0 ) ) != ( ( ( BaseType_t ) 1 ) ))
