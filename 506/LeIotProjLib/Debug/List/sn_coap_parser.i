...
sn_coap_hdr_s *sn_coap_parser_init_message(sn_coap_hdr_s *coap_msg_ptr)
{
    ...
    _memset(coap_msg_ptr, 0x00, sizeof(sn_coap_hdr_s));
...
sn_coap_options_list_s *sn_coap_parser_alloc_options(struct coap_s *handle, sn_coap_hdr_s *coap_msg_ptr)
{
    ...
    if (coap_msg_ptr->options_list_ptr == 0) {
        return 0;
    }

     
    _memset(coap_msg_ptr->options_list_ptr, 0x00, sizeof(sn_coap_options_list_s));

    coap_msg_ptr->options_list_ptr->max_age = 60;
    coap_msg_ptr->options_list_ptr->uri_port = (-1);
    coap_msg_ptr->options_list_ptr->observe = COAP_OBSERVE_NONE;
    coap_msg_ptr->options_list_ptr->accept = COAP_CT_NONE;
    coap_msg_ptr->options_list_ptr->block2 = (-1);
    coap_msg_ptr->options_list_ptr->block1 = (-1);
...
sn_coap_hdr_s *sn_coap_parser(struct coap_s *handle, uint16_t packet_data_len, uint8_t *packet_data_ptr, coap_version_e *coap_version_ptr)
{
    ...
    sn_coap_hdr_s *parsed_and_returned_coap_msg_ptr = 0;

     
    if (packet_data_ptr == 0 || packet_data_len < 4 || handle == 0) {
        return 0;
    }
    ...
    if (parsed_and_returned_coap_msg_ptr == 0) {
        return 0;
    }
...
static void sn_coap_parser_header_parse(uint8_t **packet_data_pptr, sn_coap_hdr_s *dst_coap_msg_ptr, coap_version_e *coap_version_ptr)
{
    ...
    *coap_version_ptr = (coap_version_e)(**packet_data_pptr & 0xC0);
    dst_coap_msg_ptr->msg_type = (sn_coap_msg_type_e)(**packet_data_pptr & 0x30);
    (*packet_data_pptr) += 1;
    ...
    dst_coap_msg_ptr->msg_id += **packet_data_pptr << 8;
    (*packet_data_pptr) += 2;
...
static int8_t sn_coap_parser_options_parse(struct coap_s *handle, uint8_t **packet_data_pptr, sn_coap_hdr_s *dst_coap_msg_ptr, uint8_t *packet_data_start_ptr, uint16_t packet_len)
{
    ...
    dst_coap_msg_ptr->token_len = *packet_data_start_ptr & 0x0F;
    ...
    if (dst_coap_msg_ptr->token_len) {
        ...
        if (dst_coap_msg_ptr->token_ptr == 0) {
            return -1;
        }

        _memcpy(dst_coap_msg_ptr->token_ptr, *packet_data_pptr, dst_coap_msg_ptr->token_len);
    ...
    while (message_left && (**packet_data_pptr != 0xff)) {
        ...
        uint16_t  option_number = (**packet_data_pptr >> 4);
        ...
        switch (option_number) {
            ...
            case COAP_OPTION_SIZE2:
                if (sn_coap_parser_alloc_options(handle, dst_coap_msg_ptr) == 0) {
                    return -1;
                }
                break;
        }
        ...
        switch (option_number) {
            ...
            case COAP_OPTION_PROXY_URI:
                ...
                if (dst_coap_msg_ptr->options_list_ptr->proxy_uri_ptr == 0) {
                    return -1;
                }
                _memcpy(dst_coap_msg_ptr->options_list_ptr->proxy_uri_ptr, *packet_data_pptr, option_len);
            ...
            case COAP_OPTION_URI_HOST:
                ...
                if (dst_coap_msg_ptr->options_list_ptr->uri_host_ptr == 0) {
                    return -1;
                }
                _memcpy(dst_coap_msg_ptr->options_list_ptr->uri_host_ptr, *packet_data_pptr, option_len);
                (*packet_data_pptr) += option_len;
            ...
            case COAP_OPTION_URI_PORT:
                if ((option_len > 2) || dst_coap_msg_ptr->options_list_ptr->uri_port != (-1)) {
                    return -1;
                }
                (*packet_data_pptr)++;
            ...
            case COAP_OPTION_BLOCK2:
                if ((option_len > 3) || dst_coap_msg_ptr->options_list_ptr->block2 != (-1)) {
                    return -1;
                }
                (*packet_data_pptr)++;
            ...
            case COAP_OPTION_BLOCK1:
                if ((option_len > 3) || dst_coap_msg_ptr->options_list_ptr->block1 != (-1)) {
                    return -1;
                }
                (*packet_data_pptr)++;
            ...
            case COAP_OPTION_SIZE1:
                ...
                dst_coap_msg_ptr->options_list_ptr->use_size1 = 1;
                (*packet_data_pptr)++;
            ...
            case COAP_OPTION_SIZE2:
                ...
                dst_coap_msg_ptr->options_list_ptr->use_size2 = 1;
                (*packet_data_pptr)++;
...
static int8_t sn_coap_parser_options_parse_multiple_options(struct coap_s *handle, uint8_t **packet_data_pptr, uint16_t packet_left_len,  uint8_t **dst_pptr, uint16_t *dst_len_ptr, sn_coap_option_numbers_e option, uint16_t option_number_len)
{
    ...
    uint8_t    *temp_parsed_uri_query_ptr   = 0;
    ...
    while ((temp_parsed_uri_query_ptr - *dst_pptr) < uri_query_needed_heap) {
        ...
        if (returned_option_counter > 0) {
            ...
            if (option == COAP_OPTION_URI_QUERY || option == COAP_OPTION_LOCATION_QUERY || option == COAP_OPTION_ETAG || option == COAP_OPTION_ACCEPT) {
                _memset(temp_parsed_uri_query_ptr, '&', 1);
        ...
        _memcpy(temp_parsed_uri_query_ptr, *packet_data_pptr, option_number_len);

        (*packet_data_pptr) += option_number_len;
        temp_parsed_uri_query_ptr += option_number_len;

        if ((temp_parsed_uri_query_ptr - *dst_pptr) >= uri_query_needed_heap || ((**packet_data_pptr >> 4) != 0)) {
