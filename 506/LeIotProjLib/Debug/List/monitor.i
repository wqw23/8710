#line 1 "D:\\vm-share\\506\\rtk8710_sdk\\component\\soc\\realtek\\8711b\\app\\monitor\\ram\\monitor.c"
...
#line 12 "D:\\vm-share\\506\\rtk8710_sdk\\component\\soc\\realtek\\8711b\\app\\monitor\\ram\\monitor.c"
#line 1 "D:\\vm-share\\506\\LeIotProjLib\\..\\rtk8710_sdk\\component\\soc\\realtek\\8711b\\app\\monitor\\include\\monitor_lib.h"
...
extern uint32_t CmdFlash(
      uint16_t argc,
      uint8_t  *argv[]
);


extern uint32_t CmdEfuse(
      uint16_t argc,
      uint8_t  *argv[]
);

#line 13 "D:\\vm-share\\506\\rtk8710_sdk\\component\\soc\\realtek\\8711b\\app\\monitor\\ram\\monitor.c"



void WlanNormal(   uint16_t argc,   uint8_t  *argv[]);
...
)
{
	...
	do { if ((ConfigDebugErr & 0x40000000)) DiagPrintf("\r" "\n\rRebooting ...\n\r");}while(0);
	NVIC_SystemReset();
...
{
	if (_strcmp(argv[0], "r") == 0){
		if (_strcmp(argv[1], "debug") == 0) {
			pmu_tickless_debug(1);
		} else {
			pmu_tickless_debug(0);
		}
		pmu_release_wakelock(PMU_OS);
	}

	if (_strcmp(argv[0], "a") == 0){
		pmu_acquire_wakelock(PMU_OS);
	}
	
	if (_strcmp(argv[0], "type") == 0){
	...
	if (_strcmp(argv[0], "get") == 0){
		pmu_get_wakelock_hold_stats(0);
	}
...
)
{
	...
	if (_strcmp(argv[0], "get") == 0) {
		...
		RTC_TypeDef* RTC = ((RTC_TypeDef *) 0x40003400);
		
		RTC_GetTime(((uint32_t)0x000000000), &RTC_TimeStruct);
		RTC_GetAlarm(((uint32_t)0x000000000), &RTC_AlarmStruct_temp);

		do { if ((ConfigDebugErr & 0x40000000)) DiagPrintf("\r" "time: %d:%d:%d:%d (%d) \n", RTC_TimeStruct . RTC_Days, RTC_TimeStruct . RTC_Hours, RTC_TimeStruct . RTC_Minutes, RTC_TimeStruct . RTC_Seconds, RTC_TimeStruct . RTC_H12_PMAM);}while(0);




		
		do { if ((ConfigDebugErr & 0x40000000)) DiagPrintf("\r" "alarm: %d:%d:%d:%d (%d) \n", RTC_AlarmStruct_temp . RTC_AlarmTime . RTC_Days, RTC_AlarmStruct_temp . RTC_AlarmTime . RTC_Hours, RTC_AlarmStruct_temp . RTC_AlarmTime . RTC_Minutes, RTC_AlarmStruct_temp . RTC_AlarmTime . RTC_Seconds, RTC_AlarmStruct_temp . RTC_AlarmTime . RTC_H12_PMAM);}while(0);




		
		do { if ((ConfigDebugErr & 0x40000000)) DiagPrintf("\r" "ISR: %x \n", RTC->ISR);}while(0);
		do { if ((ConfigDebugErr & 0x40000000)) DiagPrintf("\r" "ALMR1: %x \n", RTC->ALMR1);}while(0);
		do { if ((ConfigDebugErr & 0x40000000)) DiagPrintf("\r" "ALMR2: %x \n", RTC->ALMR2);}while(0);
		do { if ((ConfigDebugErr & 0x40000000)) DiagPrintf("\r" "OSC8M: %d \n", OSC8M_Get());}while(0);
	}

	if (_strcmp(argv[0], "set") == 0) {
		RTC_TimeStructInit(&RTC_TimeStruct);
		RTC_TimeStruct.RTC_Hours = Strtoul((const uint8_t*)(argv[1]), (uint8_t **)0, 10);
		RTC_TimeStruct.RTC_Minutes = Strtoul((const uint8_t*)(argv[2]), (uint8_t **)0, 10);
		RTC_TimeStruct.RTC_Seconds = Strtoul((const uint8_t*)(argv[3]), (uint8_t **)0, 10);

		if (_strcmp(argv[5], "pm") == 0) {
			RTC_TimeStruct.RTC_H12_PMAM = ((uint8_t)0x01);
		} else {
			RTC_TimeStruct.RTC_H12_PMAM = ((uint8_t)0x00);
		}
		
		RTC_SetTime(((uint32_t)0x000000000), &RTC_TimeStruct);
	}
...
{
#line 156 "D:\\vm-share\\506\\rtk8710_sdk\\component\\soc\\realtek\\8711b\\app\\monitor\\ram\\monitor.c"
...
{	
	if (_strcmp(argv[0], "set") == 0) {
		uint32_t rate = Strtoul((const uint8_t*)(argv[1]), (uint8_t **)0, 10);
		uint32_t index = 0;

		do { if ((ConfigDebugErr & 0x40000000)) DiagPrintf("\r" "UlogRate: %d, reboot after set baudrate \n", rate);}while(0);

		while (1) {
			if (0xffffffff == BAUDRATE_TABLE_40M[index][0]) {
				do { if ((ConfigDebugErr & 0x40000000)) DiagPrintf("\r" "UlogRate: Err \n");}while(0);
				break;
			}
			
			if (rate == BAUDRATE_TABLE_40M[index][0]) {
				FLASH_EreaseDwordsXIP(0x9000 + 0x30, 1);
				FLASH_TxData12BXIP(0x9000 + 0x30, 4, (uint8_t*)&rate);
		...
		do { if ((ConfigDebugErr & 0x40000000)) DiagPrintf("\r" "123456789a123456789\n");}while(0);
		delta = SYSTIMER_TickGet() - time;

		 
		do { if ((ConfigDebugErr & 0x40000000)) DiagPrintf("\r" "BaudRate: %d K\n", 200*1000*10/(delta*305));}while(0);
	}
...
const COMMAND_TABLE   UartLogRamCmdTable[] = {
	...
	{(const uint8_t*)"?",		0, CmdRamHelp,		(const uint8_t*)"\tHELP (?) : \n"
											"\t\t Print help messag\n"},
	{(const uint8_t*)"DW",		2, CmdDumpWord,	(const uint8_t*)"\tDW <Address, Hex>\n"
											"\t\t Dump memory dword or Read Hw dword register; \n"
											"\t\t Can Dump only one dword at the same time \n"
											"\t\t Unit: 4Bytes \n"},
	{(const uint8_t*)"EW",		2, CmdWriteWord,	(const uint8_t*)"\tEW <Address, Hex>\n"
											"\t\t Write memory dword or Write Hw dword register \n"
											"\t\t Can write only one dword at the same time \n"
											"\t\t Ex: EW Address Value \n"},

	{(const uint8_t*)"DB",		2, CmdDumpByte,	(const uint8_t*)"\tDW <Address, Hex>\n"
											"\t\t Dump memory byte or Read Hw byte register; \n"
											"\t\t Can Dump only one byte at the same time \n"
											"\t\t Unit: 1Bytes \n"},
	{(const uint8_t*)"EB",		2, CmdWriteByte,		(const uint8_t*)"\tEW <Address, Hex>\n"
											"\t\t Write memory byte or Write Hw byte register \n"
											"\t\t Can write only one byte at the same time \n"
											"\t\t Ex: EW Address Value \n"},
	{(const uint8_t*)"FLASH",	8, CmdFlash,			(const uint8_t*)"\tFLASH \n"
											"\t\t erase chip \n"
											"\t\t erase sector addr \n"
											"\t\t erase block addr \n"
											"\t\t read addr len \n"
											"\t\t write addr data \n"},
	{(const uint8_t*)"EFUSE",	8, CmdEfuse,			(const uint8_t*)"\tEFUSE \n"
											"\t\t wmap addr len data\n"
											"\t\t rmap \n"},
	{(const uint8_t*)"REBOOT",	4, CmdReboot,		(const uint8_t*)"\tREBOOT \n"
											"\t\t reboot \n"
											"\t\t reboot uartburn \n"},
	{(const uint8_t*)"TICKPS",	4, CmdTickPS,		(const uint8_t*)"\tTICKPS \n"
											"\t\t r: release os wakelock \n"
											"\t\t a: acquire os wakelock \n"},
	{(const uint8_t*)"RTC",		4, CmdRTC,			(const uint8_t*)"\tRTC \n"
											"\t\t get\n"},
	{(const uint8_t*)"WLAN",	4, CmdWlanNormal,	(const uint8_t*)"\tWLAN \n"
											"\t\t WLAN Driver Cmd\n"},
	{(const uint8_t*)"ULOG",	4, CmdUartLog,	(const uint8_t*)"\tULOG \n"
...
{
	uint32_t CmdCnt = 0;
	uint32_t CmdNum = (sizeof(UartLogRamCmdTable) / sizeof(COMMAND_TABLE));
...
)
{
	...
	do { if ((ConfigDebugErr & 0x40000000)) DiagPrintf("\r" "----------------- COMMAND MODE HELP ------------------\n");}while(0);
	for( LoopINdex=0  ; LoopINdex < (sizeof(UartLogRamCmdTable) / sizeof(COMMAND_TABLE)) ; LoopINdex++ )
	{
		if( UartLogRamCmdTable[LoopINdex].msg )
		{
			do { if ((ConfigDebugErr & 0x40000000)) DiagPrintf("\r" "%s\n",UartLogRamCmdTable[LoopINdex]. msg);}while(0);
		}
	}
	do { if ((ConfigDebugErr & 0x40000000)) DiagPrintf("\r" "----------------- COMMAND MODE END  ------------------\n");}while(0);
