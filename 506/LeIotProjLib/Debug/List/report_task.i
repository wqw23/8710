#line 1 "D:\\vm-share\\506\\adapter_lib\\report_task.c"
...
#line 5 "D:\\vm-share\\506\\adapter_lib\\report_task.c"
...
#line 10 "D:\\vm-share\\506\\adapter_lib\\report_task.c"
...
#line 11 "D:\\vm-share\\506\\adapter_lib\\report_task.c"

#line 1 "D:\\vm-share\\506\\adapter_lib\\report_task.h"
...
#line 13 "D:\\vm-share\\506\\adapter_lib\\report_task.c"

static unsigned int *g_rinfo = 0;
static unsigned int *g_rrinfo = 0;
static int g_count = 0;
static int g_rcount = 0;
static unsigned char g_async_flag = 0;

static SemaphoreHandle_t g_mutex;
static TaskHandle_t arHandle;
...
void _async_report_work_thread(void *arg)
{
    while(1) {
        ...
        xQueueGenericReceive( ( QueueHandle_t ) ( g_mutex ), 0, ( ( TickType_t ) 0xffffffffUL ), ( ( BaseType_t ) 0 ) );

        rcount = g_rcount;
        memset(g_rrinfo, 0, g_count * sizeof(unsigned int));
        memcpy(g_rrinfo, g_rinfo, rcount * sizeof(unsigned int));

        
        memset(g_rinfo, 0, sizeof(unsigned int) * g_count);
        g_rcount = 0;

        xQueueGenericSend( ( QueueHandle_t ) ( g_mutex ), 0, ( ( TickType_t ) 0U ), ( ( BaseType_t ) 0 ) );
...
{
    if (g_async_flag) {
        do{if(g_log_level >= -3) {printf("[E]%s"  "<%s(%d)> " "%s report task already init!\n",rtos_time_print(), __FUNCTION__, 49,__FUNCTION__);}}while(0);
        return 0;
    }

    g_rinfo = (unsigned int *) pvPortMalloc(sizeof(unsigned int) * count);
    if (g_rinfo == 0) {
        return -1;
    }

    g_rrinfo = (unsigned int *) pvPortMalloc(sizeof(unsigned int) * count);
    if (g_rrinfo == 0) {
        vPortFree(g_rinfo);
        g_rinfo = 0;
        return -1;
    ...
    g_count = count;
    g_mutex = xQueueCreateMutex( ( ( uint8_t ) 1U ) );
    xTaskGenericCreate( ( _async_report_work_thread ), ( "async_report" ), ( 512 ), ( 0 ), ( 4 ), ( &arHandle ), ( 0 ), ( 0 ) );

    
    g_async_flag = 1;

    do{if(g_log_level >= 2) {printf("[D]%s"  "%s report task init success!\n",rtos_time_print(),__FUNCTION__);}}while(0);
    return 0;
}
...
{
    if (g_async_flag == 0) {
        do{if(g_log_level >= -3) {printf("[E]%s"  "<%s(%d)> " "%s report task not init!\n",rtos_time_print(), __FUNCTION__, 80,__FUNCTION__);}}while(0);
        return 0;
    }
    ...
    xQueueGenericSend( ( QueueHandle_t ) ( g_mutex ), 0, ( ( TickType_t ) 0U ), ( ( BaseType_t ) 0 ) );
    vQueueDelete( ( QueueHandle_t ) ( g_mutex ) );

    g_async_flag = 0;

    do{if(g_log_level >= 2) {printf("[D]%s"  "%s report task deinit success!\n",rtos_time_print(),__FUNCTION__);}}while(0);
    return 0;
}
...
void async_report_attr(unsigned int attr_id, REPORT_FLASH_FLAG flag)
{
    ...
    if (g_async_flag) {
        ...
        if (i < g_count) {
            ...
            do{if(g_log_level >= -3) {printf("[E]%s"  "<%s(%d)> " "%s async report count over set max value!\n",rtos_time_print(), __FUNCTION__, 125,__FUNCTION__);}}while(0);
        }
OUT:
        xQueueGenericSend( ( QueueHandle_t ) ( g_mutex ), 0, ( ( TickType_t ) 0U ), ( ( BaseType_t ) 0 ) );
    }
