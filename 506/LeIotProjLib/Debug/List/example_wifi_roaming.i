#line 1 "D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\example\\wifi_roaming\\example_wifi_roaming.c"
...
#line 4 "D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\example\\wifi_roaming\\example_wifi_roaming.c"
...
#line 5 "D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\example\\wifi_roaming\\example_wifi_roaming.c"
...
#line 6 "D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\example\\wifi_roaming\\example_wifi_roaming.c"
...
#line 7 "D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\example\\wifi_roaming\\example_wifi_roaming.c"
...
#line 8 "D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\example\\wifi_roaming\\example_wifi_roaming.c"
...
#line 9 "D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\example\\wifi_roaming\\example_wifi_roaming.c"

#line 16 "D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\example\\wifi_roaming\\example_wifi_roaming.c"
...
static wifi_roaming_ap_t * ap_list[3]={0};
...
uint32_t wifi_roaming_find_ap_from_scan_buf(char*buf, int buflen, char *target_ssid, void *user_data)
{
	...
	while(plen < buflen)
	{
		uint32_t len, ssid_len, i, security_mode, security_type, channel;
		int32_t rssi;
		uint8_t *mac, *ssid;				
		...
		((pwifi->security_type & (0x00400000|0x00200000))&&(security_type & (0x00400000|0x00200000)))){
			if(ap_count < 3){
				candicate = (wifi_roaming_ap_t *)pvPortMalloc(sizeof(wifi_roaming_ap_t));
				if(!candicate)
				{
					rtl_printf("\r\n malloc buf for AP info fail!");
					break;
				}
				_memset(candicate, 0, sizeof(wifi_roaming_ap_t));
				_memcpy(candicate ->ssid, ssid, ssid_len);
				_memcpy(candicate ->bssid, mac, 6);
				candicate->channel = channel;
				candicate->security_type = security_type;
				_memcpy(candicate ->password, pwifi->password, prvStrLen((const unsigned char *) pwifi->password));
...
void wifi_roaming_thread(void *param)
{
	...
	uint8_t	pscan_config = 0x01;
	_memset(&setting, 0, sizeof(rtw_wifi_setting_t));
	_memset(&roaming_ap, 0, sizeof(wifi_roaming_ap_t));
	roaming_ap.rssi = -100;
		...
		if(wifi_is_up(RTW_STA_INTERFACE)&&(RTW_SUCCESS == wifi_is_ready_to_transceive(RTW_STA_INTERFACE)))
		{
			...
			if( (ap_rssi < -65))
			{
				if( polling_count >= (5-1))
				{
					...
					_strcpy(roaming_ap . ssid, setting . ssid);
					roaming_ap.security_type =  setting.security_type;
					_strcpy(roaming_ap . password, setting . password);
					roaming_ap.key_idx = setting.key_idx;
					
					if(RTW_ERROR == wifi_get_ap_bssid(roaming_ap.bssid))
						rtl_printf("\r\n get AP BSSID FAIL!");

					_memcpy(roaming_ap . ip, IP, 4);

					wifi_scan_networks_with_ssid(wifi_roaming_find_ap_from_scan_buf, (void *)&roaming_ap, 500, roaming_ap.ssid, prvStrLen((const unsigned char *) roaming_ap . ssid));

					wifi_set_autoreconnect(0);

					i = 0;
connect_ap:
					if(ap_list[i] && _memcmp(bssid_ap, ap_list[i]->bssid, 6))
					{									
						...
						while(1)
						{
							...
							if( RTW_SUCCESS == wifi_connect_bssid(ap_list[i]->bssid, ap_list[i]->ssid,ap_list[i]->security_type,
								ap_list[i]->password, 6, prvStrLen((const unsigned char *) ap_list[i]->ssid), prvStrLen((const unsigned char *) ap_list[i]->password), ap_list[i]->key_idx, 0 ))
							{

								wifi_reg_event_handler(WIFI_EVENT_IP_CHANGED, wifi_ip_changed_hdl, 0);

								LwIP_DHCP(0, DHCP_START);
								
								if(_memcmp(roaming_ap . ip, IP, 4))
								{
									wifi_indication(WIFI_EVENT_IP_CHANGED, 0,0, 0);
					...
					while(ap_count)
					{
						vPortFree (ap_list[ap_count-1]);
						ap_count--;
					}
					
					_memset(ap_list, 0, sizeof(ap_list));
					polling_count = 0;			
...
{
	if(xTaskGenericCreate( ( wifi_roaming_thread ), ( ((const char*)"wifi_roaming_thread") ), ( 1024 ), ( 0 ), ( ( ( UBaseType_t ) 0U ) + 1 ), ( 0 ), ( 0 ), ( 0 ) ) != ( ( ( BaseType_t ) 1 ) ))
