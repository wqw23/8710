#line 1 "D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\application\\xmodem\\uart_fw_update.c"
...
#line 1 "D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\application\\xmodem\\xmodem.h"
...
#line 11 "D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\application\\xmodem\\xmodem.h"
...
#line 26 "D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\application\\xmodem\\xmodem.h"



 
#line 37 "D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\application\\xmodem\\xmodem.h"



 
#line 49 "D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\application\\xmodem\\xmodem.h"
...
typedef struct _XMODEM_CTRL_ {
    ...
    uint32_t rx_len;
    char *pXFrameBuf;    
    uint32_t (*RxFrameHandler)(char *ptr,  uint32_t frame_num, uint32_t frame_size);
    XMODEM_COM_PORT ComPort;
}XMODEM_CTRL, *PXMODEM_CTRL;

typedef uint32_t (*RxFrameHandler_t)(char *ptr,  uint32_t frame_num, uint32_t frame_size);

extern int16_t xModemStart(XMODEM_CTRL *pXMCtrl, char *FrameBuf, RxFrameHandler_t RxFrameHdl);
extern int16_t xModemEnd(XMODEM_CTRL *pXMCtrl);
extern int32_t _xModemRxBuffer(XMODEM_CTRL *pXMCtrl, int32_t MaxSize);



#line 374 "D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\application\\xmodem\\uart_fw_update.c"
...
#line 375 "D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\application\\xmodem\\uart_fw_update.c"
...
#line 376 "D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\application\\xmodem\\uart_fw_update.c"
...
#line 377 "D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\application\\xmodem\\uart_fw_update.c"

extern char xmodem_uart_readable(void);
extern char xmodem_uart_getc(void);
extern void xmodem_uart_putc(char c);

char xMFrameBuf[1024];
XMODEM_CTRL _xMCtrl;

extern const update_file_img_id OtaImgId[2];
static update_ota_target_hdr OtaTargetHdr;
static uint32_t fw_img2_addr;
static uint32_t flash_wr_err_cnt;
static int SigCnt;
static uint8_t signature[9];
static update_dw_info DownloadInfo[2];
static int ImageCnt;
static uint32_t OtaFg;
static int32_t RemainBytes;
static uint32_t i;
static uint32_t TempLen;
static int32_t OtaImgSize;
static int size;

void xDelayUs(uint32_t us)
{
	DelayUs(us);
}
...
int32_t
xModem_Init_UART_Port(uint8_t uart_idx, uint8_t pin_mux, uint32_t baud_rate)
{
...
void
xModem_DeInit_UART_Port(uint8_t uart_idx)
{
	xmodem_uart_deinit();
}

int GetDownloadInfo(uint32_t addr, update_ota_target_hdr * pOtaTgtHdr)
{
	uint32_t ImageCnt;
	...
	if(pOtaTgtHdr->RdpStatus == 1) {
		...
		if(pOtaTgtHdr->FileImgHdr.Offset < pOtaTgtHdr->FileRdpHdr.Offset) {
			...
			DownloadInfo[1].FlashAddr = 0x081DC000 - 0x08000000;
			...
			DownloadInfo[0].FlashAddr = 0x081DC000 - 0x08000000;
...
uint32_t xModem_Frame_Img2(char *ptr,  unsigned int frame_num, unsigned int frame_size)
{
	uint32_t uart_ota_target_index = 1;
	uint32_t fw_img2_size;
	uint8_t *pImgId = 0;
	uint32_t IncFg = 0;
	flash_t flash;
	int read_bytes;
	...
	rtl_printf("\rframe_num: %d frame_size: %d", frame_num, frame_size);
	if (flash_wr_err_cnt)
		return 0;
	...
	if (frame_num == 1) {
		...
		if (ota_get_cur_index() == 0) {
			uart_ota_target_index = 1;
			rtl_printf("\n\rOTA2 address space will be upgraded\n");
		} else {
			uart_ota_target_index = 0;
			rtl_printf("\n\rOTA1 address space will be upgraded\n");
		}		
		pImgId = (uint8_t *)&OtaImgId[uart_ota_target_index];
		
		 
		 
		if(!get_ota_tartget_header((uint8_t*)ptr, frame_size, &OtaTargetHdr, pImgId)){
		...
		if(OtaTargetHdr.RdpStatus == 1) {
			...
			flash_erase_sector(&flash, 0x081DC000 - 0x08000000);
...
)
{
	...
	if(verify_ota_checksum(fw_img2_addr, OtaImgSize, signature, &OtaTargetHdr)){
		...
		device_mutex_lock(RT_DEV_LOCK_FLASH);
		flash_erase_sector(&flash, fw_img2_addr - 0x08000000);
		device_mutex_unlock(RT_DEV_LOCK_FLASH);
...
void OTU_FW_Update(uint8_t uart_idx, uint8_t pin_mux, uint32_t baud_rate)
{
	uint32_t wr_len = 0;
	int ret = 1;
	_memset(signature, 0, sizeof(signature));
	_memset(&OtaTargetHdr, 0, sizeof(OtaTargetHdr));
	_memset((uint8_t *)&DownloadInfo, 0, 2*sizeof(update_dw_info));
	fw_img2_addr = 0;
	flash_wr_err_cnt = 0;
	SigCnt = 0;
	...
	rtl_printf("FW Update Over UART%d, PinMux=%d, Baud=%d\r\n", uart_idx, pin_mux, baud_rate);
	...
	rtl_printf("FW Update Image2 @ 0x%x\r\n", fw_img2_addr);
