#line 1 "D:\\vm-share\\506\\adapter\\wifi_utils.c"
...
#line 5 "D:\\vm-share\\506\\adapter\\wifi_utils.c"
...
#line 6 "D:\\vm-share\\506\\adapter\\wifi_utils.c"
...
#line 9 "D:\\vm-share\\506\\adapter\\wifi_utils.c"
...
#line 11 "D:\\vm-share\\506\\adapter\\wifi_utils.c"
...
#line 12 "D:\\vm-share\\506\\adapter\\wifi_utils.c"
...
#line 13 "D:\\vm-share\\506\\adapter\\wifi_utils.c"
...
#line 14 "D:\\vm-share\\506\\adapter\\wifi_utils.c"
...
#line 16 "D:\\vm-share\\506\\adapter\\wifi_utils.c"
...
#line 17 "D:\\vm-share\\506\\adapter\\wifi_utils.c"
...
#line 18 "D:\\vm-share\\506\\adapter\\wifi_utils.c"

int factory_mode = 0;
static int scan_wifi_finish = 0;
...
UINT8 connect_wifi(char *ssid,char *passwd)  
{
    ...
    _memset(&config, 0, sizeof(config));
    _memcpy(config . ssid, ssid, prvStrLen((const unsigned char *) ssid));
    _memcpy(config . password, passwd, prvStrLen((const unsigned char *) passwd));
    do{if(g_log_level >= 0) {rtl_printf("[D]%s"  "connect_ssid is ok\n",rtos_time_print());}}while(0);
    if(wifi_on(RTW_MODE_STA) < 0)
        do{if(g_log_level >= 0) {rtl_printf("[D]%s"  "\n\r[WLAN_SCENARIO_EXAMPLE] ERROR: wifi_on failed\n",rtos_time_print());}}while(0);

    do{
        val=wifi_connect(config.ssid,RTW_SECURITY_WPA2_AES_PSK,config.password,prvStrLen((const unsigned char *) config . ssid),prvStrLen((const unsigned char *) config . password),-1,0);
        if(val != RTW_SUCCESS)
        {
            do{if(g_log_level >= -3) {rtl_printf("[E]%s"  "<%s(%d)> " "wifi_station_connect error!!!%d\n",rtos_time_print(), __FUNCTION__, 41,count);}}while(0);
            cycle_flag=0;
        }else{
            do{if(g_log_level >= 0) {rtl_printf("[D]%s"  "connect to %s success!!!!!\n",rtos_time_print(),config . ssid);}}while(0);
    ...
    {
        do{
            if(IOTWifiP_NetworkIsOK())
            {
                do{if(g_log_level >= 2) {rtl_printf("[D]%s"  "connect_wifi_PASSWD_success !PASSWD=%s\n",rtos_time_print(),config . password);}}while(0);
                return 1;
            }else{
                do{if(g_log_level >= 2) {rtl_printf("[D]%s"  "connect_continue!!!!!!%d--IOTWifiP_NetworkIsOK()=%d\n",rtos_time_print(),cycle_flag_count,IOTWifiP_NetworkIsOK());}}while(0);
                vTaskDelay(1000/( ( TickType_t ) 1000 / ( ( uint32_t ) 1000 ) ));
            }
        }while(--cycle_flag_count);
    }
    do{if(g_log_level >= 2) {rtl_printf("[D]%s"  "connect_wifi_error!!!%d\n",rtos_time_print(),cycle_flag_count);}}while(0);
    return 0;
}
...
static rtw_result_t scan_result_handler(rtw_scan_handler_result_t* malloced_scan_result)
{
    if (malloced_scan_result->scan_complete != RTW_TRUE) {
        ...
        do{if(g_log_level >= 0) {rtl_printf("[D]%s"  "ssid is %s\n",rtos_time_print(),record->SSID . val);}}while(0);
        do{if(g_log_level >= 0) {rtl_printf("[D]%s"  "record->signal_strength is %d\n",rtos_time_print(),record->signal_strength);}}while(0);
        if ((prvStrCmp((const unsigned char *) record->SSID . val, (const unsigned char *) "Lim_Fac-Mode")) == 0) {
            do{if(g_log_level >= 0) {rtl_printf("[D]%s"  "factory wifi ssid is %s is found !!! \n",rtos_time_print(),record->SSID . val);}}while(0);
            factory_mode = 1;
            scan_wifi_finish = 1;
            return RTW_SUCCESS;
        }
...
UINT8 isFactoryMode_wifi(void)
{
    ...
    do{if(g_log_level >= 0) {rtl_printf("[D]%s"  "isFactoryMode_wifi \n",rtos_time_print());}}while(0);
    scan_wifi_finish = 0;

    if(wifi_scan_networks(scan_result_handler, 0) != RTW_SUCCESS){
        do{if(g_log_level >= 0) {rtl_printf("[D]%s"  "\n\r[WLAN_SCENARIO_EXAMPLE] ERROR: wifi_scan_networks() failed\n",rtos_time_print());}}while(0);
    }

    while(!scan_wifi_finish){
        do{if(g_log_level >= 2) {rtl_printf("[D]%s"  "scan task running....\n",rtos_time_print());}}while(0);
        vTaskDelay(1000/( ( TickType_t ) 1000 / ( ( uint32_t ) 1000 ) ));
    }
