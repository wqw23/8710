#line 1 "D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\mbed\\targets\\hal\\rtl8711b\\efuse_api.c"
...
#line 18 "D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\mbed\\targets\\hal\\rtl8711b\\efuse_api.c"
...
#line 20 "D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\mbed\\targets\\hal\\rtl8711b\\efuse_api.c"
...
static uint8_t rdp_bin_test_plaintext[16]= {0x08,0x18,0x70,0x47,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff};
 
static uint8_t rdp_bin_test_enc[16] = {0};
 
static uint32_t rdp_bin_checksum = 0;
 
static uint8_t * RDP_key = 0;

static uint32_t EFUSE_OTP_Check(uint16_t Addr, uint8_t Data)
{
	uint8_t L25OutVoltage = 7;
	uint32_t CtrlSetting = ((uint32_t)(*((volatile uint32_t*)(0x40000000 + 0x00E8))));
	uint32_t bResult = 1;
	uint8_t temp = 0xFF;

	EFUSE_WRITE_Check_ACUT(1);
	if ((Addr >= (0x80)) && (Addr < (0x80 + 0x20))){
...
int  efuse_mtp_write(uint8_t *data, uint8_t len)
{
	...
	uint8_t len_low, section_num, word_enable = 0;
	uint8_t ret = 0;
	...
	switch (section_num){
		case 0:
			ret = EFUSE_USER1_Write_ACUT(0, word_enable, data);		if (ret == 0) return -1;
			break;
			
		case 1:
			ret = EFUSE_USER1_Write_ACUT(0, 0xf, data);				if (ret == 0) return -1;
			ret = EFUSE_USER1_Write_ACUT(1, word_enable, data+8);	if (ret == 0) return -1;
			break;

		case 2:
			ret = EFUSE_USER1_Write_ACUT(0, 0xf, data);				if (ret == 0) return -1;
			ret = EFUSE_USER1_Write_ACUT(1, 0xf, data+8);			if (ret == 0) return -1;
			ret = EFUSE_USER1_Write_ACUT(2, word_enable, data+8);	if (ret == 0) return -1;
			break;

		case 3:
			ret = EFUSE_USER1_Write_ACUT(0, 0xf, data);				if (ret == 0) return -1;
			ret = EFUSE_USER1_Write_ACUT(1, 0xf, data+8);			if (ret == 0) return -1;
			ret = EFUSE_USER1_Write_ACUT(2, 0xf, data+16);			if (ret == 0) return -1;
			ret = EFUSE_USER1_Write_ACUT(3, word_enable, data+24);	if (ret == 0) return -1;
			break;
		...
		case 4:
			...
			ret = EFUSE_USER1_Write_ACUT(1, 0xf, data+8);			if (ret == 0) return -1;
			ret = EFUSE_USER1_Write_ACUT(2, 0xf, data+16);			if (ret == 0) return -1;
			ret = EFUSE_USER1_Write_ACUT(3, 0xf, data+24);			if (ret == 0) return -1;
	}		
	return len;
}
...
int efuse_otp_write(uint8_t address, uint8_t len, uint8_t *buf)
{
	...
	ret = EFUSE_OTP_Write32B_ACUT(content);
...
int efuse_otp_chk(uint8_t len, uint8_t *buf)
{
	...
	for (Idx = 0; Idx < 0x20; Idx++){
		bResult = EFUSE_OTP_Check((0x80+Idx), (*(content+Idx)));
		if (bResult != 1){
			break;
		}
	}
...
void efuse_rdp_check_image_encrypt(uint8_t* SW_Key)
{
	...
	pByte = (uint8_t *)&rdp_bin_checksum;
	...
	do { if ((ConfigDebugErr & 0x40000000)) DiagPrintf("\r" "[%s] rdp_bin_checksum = %x\n", __func__, rdp_bin_checksum);}while(0);
}
...
void efuse_rdp_check(uint8_t* SW_key)
{
	uint32_t temp = 0;
	uint32_t ret = (!0);
	SYSTEM_DATA* pSysData = (SYSTEM_DATA*)(0x08000000 + 0x9000);
	uint32_t RDPFlashAddr = pSysData->RDP_FlashAddr;
	uint32_t RDPLen = pSysData->RDP_Len;
	uint32_t checksum_ipsec, checksum_rdp_flash;
	uint8_t aes_ecb_key_dummy[16]={0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 
					 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
	uint8_t *rdp_bin_data_en = (uint8_t*)((uint32_t)__rom_top_4k_start_ - 16 * 1024);
	uint32_t PollTimes;
	uint32_t RdpEnable = 0;
	uint32_t WriteKeyState = 0;
	uint32_t RDP_ERR_Flag = 0;

	EFUSE_WRITE_CHECK(1);

	 
	for(PollTimes = 0; PollTimes < (3); PollTimes++) {
		 
		ret = RDP_EN_Request();

		if (ret == 0) {
			do { if ((ConfigDebugErr & 0x40000000)) DiagPrintf("\r" "BIT_RDP_EN_LOAD timeout.\n");}while(0);
			RDP_ERR_Flag = ((uint32_t)0x00000003);
			goto exit;
		}

		temp = ((uint32_t)(*((volatile uint32_t*)(0x40000000 + 0x02FC))));

		if (temp & (1<<(3))) {
			RdpEnable = 1;
			do { if ((ConfigDebugErr & 0x40000000)) DiagPrintf("\r" "[%s] PG RDP enable bit ok: %d\n", __func__, PollTimes);}while(0);
			break;
		} else {
			EFUSE_RDP_EN();
			do { if ((ConfigDebugErr & 0x40000000)) DiagPrintf("\r" "[%s] PG RDP enable bit again: %d\n", __func__, PollTimes);}while(0);
		}
	}
	if(RdpEnable == 0) {
		do { if ((ConfigDebugErr & 0x40000000)) DiagPrintf("\r" "PG RDP enable bit fail!\n");}while(0);
		goto exit;
	}
	...
	if(SW_key != 0) {
		...
		RDPFlashAddr = (uint32_t)&rdp_bin_test_enc ;
		RDPLen = 16;
		checksum_rdp_flash = rdp_bin_checksum;
		do { if ((ConfigDebugErr & 0x40000000)) DiagPrintf("\r" "software key is valid, use firmware in ram to check RDP-key!\n");}while(0);
	} else {
		checksum_rdp_flash = ((uint32_t)(*((volatile uint32_t*)(RDPFlashAddr + RDPLen))));
		do { if ((ConfigDebugErr & 0x40000000)) DiagPrintf("\r" "software key is not NULL, use the 4KB firmware in flash to check RDP-key!\n");}while(0);
	}

	if(RDP_key == 0) {
		do { if ((ConfigDebugErr & 0x40000000)) DiagPrintf("\r" "[%s]: RDP key is NULL\n", __func__);}while(0);
		goto exit;
	}
	...
	if (RdpEnable) {
		for(PollTimes = 0; PollTimes < (3); PollTimes++) {
			do { if ((ConfigDebugErr & 0x40000000)) DiagPrintf("\r" "BOOT_FLASH_RDP RDP enable \n");}while(0);		
			...
			if(0xFFFFFFFF == *((uint32_t*)rdp_bin_data_en)) {
				do { if ((ConfigDebugErr & 0x40000000)) DiagPrintf("\r" "RDP.bin Empty.\n");}while(0);
				RDP_ERR_Flag = ((uint32_t)0x00000002);
				goto exit;
			}
			...
			rtl_crypto_aes_ecb_decrypt(rdp_bin_data_en,  RDPLen , 0, 0, (uint8_t*)0x1003F000); 
			...
			if(checksum_rdp_flash == checksum_ipsec){
				WriteKeyState = 1;
				do { if ((ConfigDebugErr & 0x40000000)) DiagPrintf("\r" "[%s] PG write RDP key ok: %d\n", __func__, PollTimes);}while(0);
				do { if ((ConfigDebugErr & 0x40000000)) DiagPrintf("\r" "RDP bin decryption OK!\n");}while(0);
				break;
			}else{
				EFUSE_RDP_KEY(RDP_key);
				do { if ((ConfigDebugErr & 0x40000000)) DiagPrintf("\r" "write RDP key again: %d \n", PollTimes);}while(0);
				do { if ((ConfigDebugErr & 0x40000000)) DiagPrintf("\r" "checksum_ipsec = 0x%x, checksum_rdp_flash = 0x%x\n", checksum_ipsec, checksum_rdp_flash);}while(0);
				RDP_ERR_Flag = ((uint32_t)0x00000005);
			}
		}

		if(WriteKeyState == 0) {
			do { if ((ConfigDebugErr & 0x40000000)) DiagPrintf("\r" "write RDP key failed\n");}while(0);
			goto exit;
		}
		
	} else {
		RDP_ERR_Flag = ((uint32_t)0x00000004);
		goto exit;
	}
	
exit:
	if(RDP_ERR_Flag != 0) {
		do { if ((ConfigDebugErr & 0x40000000)) DiagPrintf("\r" "RDP_ERR_Flag: %d\n", RDP_ERR_Flag);}while(0);
	}
	EFUSE_WRITE_CHECK(0);
}
...
void efuse_otf_cmd(uint32_t NewState)
{
	uint32_t value = ((uint32_t)(*((volatile uint32_t*)(0x40000000 + 0x0038))));
	uint8_t ret = 1;
	...
	ret = EFUSE_LogicalMap_Write_Check(0x18, 2, (uint8_t*)&value);
	if (ret == 0) {
		do { if ((ConfigDebugErr & 0x40000000)) DiagPrintf("\r" "Set OTF control bit fail \n");}while(0);
	}
}
