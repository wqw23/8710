#line 1 "D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\mbed\\targets\\hal\\rtl8711b\\flash_api.c"
...
#line 21 "D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\mbed\\targets\\hal\\rtl8711b\\flash_api.c"
...
#line 25 "D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\mbed\\targets\\hal\\rtl8711b\\flash_api.c"
...
#line 29 "D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\mbed\\targets\\hal\\rtl8711b\\flash_api.c"
...
static void flash_write_unlock()
{
	uint8_t i = 0;
	for(;i < 8; i++){
		((NVIC_Type *) ((0xE000E000UL) + 0x0100UL) )->ISER[i] = ISER[i];
	}
	
	((SysTick_Type *) ((0xE000E000UL) + 0x0010UL) )->CTRL = systick_ctrl;
}
...
int flash_get_status(flash_t *obj)
{
	...
	flash_write_lock();
	FLASH_RxCmd(flash_init_para.FLASH_cmd_rd_status, 1, (uint8_t*)&data);
	flash_write_unlock();
...
int  flash_read_word(flash_t *obj, uint32_t address, uint32_t * data)
{
	...
	((data != 0) ? (void)0 : io_assert_failed((uint8_t *)"D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\mbed\\targets\\hal\\rtl8711b\\flash_api.c", 198));
	
	uint32_t offset_to_align = address & 0x03;
	uint32_t read_data;
	uint32_t temp;
	uint32_t i = 4 - offset_to_align;

	if(offset_to_align){
		address -= offset_to_align;
		temp = ((uint32_t)(*((volatile uint32_t*)(0x08000000 + address))));
		...
		* data = ((uint32_t)(*((volatile uint32_t*)(0x08000000 + address))));
	}
...
int  flash_write_word(flash_t *obj, uint32_t address, uint32_t data)
{
	...
	if(offset_to_align){
		...
		FLASH_TxData12B(address, 4, (uint8_t*)&temp);
		...
		FLASH_TxData12B(address, 4, (uint8_t*)&write_word);
	}
...
int  flash_stream_read(flash_t *obj, uint32_t address, uint32_t len, uint8_t * data)
{
	((data != 0) ? (void)0 : io_assert_failed((uint8_t *)"D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\mbed\\targets\\hal\\rtl8711b\\flash_api.c", 275));
	...
	if (offset_to_align != 0) {
		...
		read_word = ((uint32_t)(*((volatile uint32_t*)(0x08000000 + (address - offset_to_align)))));
	...
	if ((uint32_t)pbuf & 0x03) {
		while (len >= 4) {
			read_word = ((uint32_t)(*((volatile uint32_t*)(0x08000000 + address))));
			for (i=0;i<4;i++) {
				*pbuf = *(ptr+i);
				pbuf++;
			}
		...
		while (len >= 4) {
			*((uint32_t *)pbuf) = ((uint32_t)(*((volatile uint32_t*)(0x08000000 + address))));
			pbuf += 4;
			address += 4;
			len -= 4;
		}
...
int  flash_stream_write(flash_t *obj, uint32_t address, uint32_t len, uint8_t * data)
{
	...
	while(page_cnt){	
		...
		if(offset_to_align != 0){
			read_word = ((uint32_t)(*((volatile uint32_t*)(0x08000000 + addr_begin - offset_to_align))));
		...
		if(size > 0){
			read_word = ((uint32_t)(*((volatile uint32_t*)(0x08000000 + addr_begin))));
			for( i = 0;i < size;i++){
				read_word = (read_word & (~(0xff << (8*i)))) | ((*buffer) <<(8*i));
				buffer++;
			}
...
int flash_set_extend_addr(flash_t *obj, uint32_t data)
{
	...
	uint8_t segnum = (uint8_t)(data & 0x07);
...
int flash_read_id(flash_t *obj, uint8_t *buf, uint8_t len)
{
	((buf != 0) ? (void)0 : io_assert_failed((uint8_t *)"D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\mbed\\targets\\hal\\rtl8711b\\flash_api.c", 495));
	((len >= 3) ? (void)0 : io_assert_failed((uint8_t *)"D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\mbed\\targets\\hal\\rtl8711b\\flash_api.c", 496));
