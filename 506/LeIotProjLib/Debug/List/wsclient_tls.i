...
#line 4 "D:\\vm-share\\506\\LeIotProjLib\\..\\rtk8710_sdk\\component\\common\\network\\websocket/libwsclient.h"
...
static x509_crt* cacert  = 0;
static int my_verify(void *data, x509_crt *crt, int depth, int *flags) 
{
	char buf[1024];
	((void) data);

	rtl_printf("Verify requested for (Depth %d):\n", depth);
	x509_crt_info(buf, sizeof(buf) - 1, "", crt);
	rtl_printf("%s", buf);

	if(((*flags) & 0x01) != 0)
		rtl_printf("server certificate has expired\n");

	if(((*flags) & 0x02) != 0)
		rtl_printf("  ! server certificate has been revoked\n");

	if(((*flags) & 0x04) != 0)
		rtl_printf("  ! CN mismatch\n");

	if(((*flags) & 0x08) != 0)
		rtl_printf("  ! self-signed or not signed by a trusted CA\n");

	if(((*flags) & 0x10) != 0)
		rtl_printf("  ! CRL not trusted\n");

	if(((*flags) & 0x20) != 0)
		rtl_printf("  ! CRL expired\n");

	if(((*flags) & 0x0100) != 0)
		rtl_printf("  ! other (unknown) flag\n");

	if((*flags) == 0)
		rtl_printf("  Certificate verified without error flags\n");
...
void *wss_tls_connect(int *sock , char *host, int port){
	...
	int ret;
	struct wss_tls *tls =0;

	memory_set_own(pvPortMalloc, vPortFree);
	tls = (struct wss_tls *) pvPortMalloc(sizeof(struct wss_tls));

	if(tls){
		ssl_context *ssl = &tls->ctx;
		_memset(tls, 0, sizeof(struct wss_tls));
		...
		cacert = (struct _x509_crt *)pvPortMalloc(sizeof(x509_crt));
		if(!cacert){
			rtl_printf("Error: malloc cacert struct fail!");
			goto exit;
		}
		...
		ssl_set_authmode(ssl, 1);
		
		x509_crt_parse(cacert, g_pszCrt, g_iCrtLen);
		ssl_set_ca_chain(ssl, cacert, 0, 0);
		ssl_set_verify( ssl, my_verify, 0 );

		ssl_set_rng(ssl, ws_random, 0);
...
exit:
	...
	if(cacert){
		...
		rom_ssl_ram_map . ssl_free(cacert);
		vPortFree(cacert);
		cacert = 0;
	}
...
#line 220 "D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\network\\websocket\\wsclient_tls.c"
}
...
int wss_tls_handshake(void *tls_in){
	...
	return ret;
#line 250 "D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\network\\websocket\\wsclient_tls.c"
}
...
#line 287 "D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\network\\websocket\\wsclient_tls.c"
}
