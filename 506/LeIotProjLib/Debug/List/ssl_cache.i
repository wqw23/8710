...
#line 1 "D:\\vm-share\\506\\LeIotProjLib\\..\\rtk8710_sdk\\component\\common\\network\\ssl\\polarssl-1.3.8\\include\\polarssl/ssl_cache.h"
...
#line 126 "D:\\vm-share\\506\\LeIotProjLib\\..\\rtk8710_sdk\\component\\common\\network\\ssl\\polarssl-1.3.8\\include\\polarssl/ssl_cache.h"
...
void ssl_cache_init( ssl_cache_context *cache )
{
    _memset(cache, 0, sizeof( ssl_cache_context ));
...
int ssl_cache_get( void *data, ssl_session *session )
{
    ...
    while( cur != 0 )
    {
        ...
        if( _memcmp(session->id, entry->session . id, entry->session . length) != 0 )

            continue;

        _memcpy(session->master, entry->session . master, 48);
        ...
        if( entry->peer_cert.p != 0 )
        {
            session->peer_cert =
                (x509_crt *) rom_ssl_ram_map . ssl_malloc( sizeof(x509_crt) );
...
#line 173 "D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\network\\ssl\\polarssl-1.3.8\\library\\ssl_cache.c"
        ...
        if( _memcmp(session->id, cur->session . id, cur->session . length) == 0 )
            break;  

#line 184 "D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\network\\ssl\\polarssl-1.3.8\\library\\ssl_cache.c"
    ...
    if( cur == 0 )
    {
#line 206 "D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\network\\ssl\\polarssl-1.3.8\\library\\ssl_cache.c"
        ...
        else
        {
            ...
            cur = (ssl_cache_entry *)
                        rom_ssl_ram_map . ssl_malloc( sizeof(ssl_cache_entry) );
            if( cur == 0 )
            {
                ret = 1;
                goto exit;
            ...
            _memset(cur, 0, sizeof(ssl_cache_entry));
    ...
    _memcpy(&cur->session, session, sizeof( ssl_session ));


    

 
    if( cur->peer_cert.p != 0 )
    {
        rom_ssl_ram_map . ssl_free( cur->peer_cert.p );
        _memset(&cur->peer_cert, 0, sizeof(x509_buf));
    }
    ...
    if( session->peer_cert != 0 )
    {
        cur->peer_cert.p = (unsigned char *)
                                rom_ssl_ram_map . ssl_malloc( session->peer_cert->raw.len );
        if( cur->peer_cert.p == 0 )
        {
            ret = 1;
            goto exit;
        ...
        _memcpy(cur->peer_cert . p, session->peer_cert->raw . p, session->peer_cert->raw . len);

        cur->peer_cert.len = session->peer_cert->raw.len;

        cur->session.peer_cert = 0;
    }
...
void ssl_cache_free( ssl_cache_context *cache )
{
    ...
    while( cur != 0 )
    {
        ...
        rom_ssl_ram_map . ssl_free( prv->peer_cert.p );


        rom_ssl_ram_map . ssl_free( prv );
    }
