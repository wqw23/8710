#line 1 "D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\mbed\\targets\\hal\\rtl8711b\\spi_api.c"
...
#line 18 "D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\mbed\\targets\\hal\\rtl8711b\\spi_api.c"
#line 1 "D:\\vm-share\\506\\LeIotProjLib\\..\\rtk8710_sdk\\component\\common\\mbed\\hal\\spi_api.h"
...
#line 29 "D:\\vm-share\\506\\LeIotProjLib\\..\\rtk8710_sdk\\component\\common\\mbed\\hal\\spi_api.h"
...
#line 19 "D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\mbed\\targets\\hal\\rtl8711b\\spi_api.c"
#line 1 "D:\\vm-share\\506\\LeIotProjLib\\..\\rtk8710_sdk\\component\\common\\mbed\\hal_ext\\spi_ex_api.h"
...
enum {
    SPI_CS_TOGGLE_EVERY_FRAME=0,       
    SPI_CS_TOGGLE_START_STOP=1         
};
...
#line 20 "D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\mbed\\targets\\hal\\rtl8711b\\spi_api.c"
...
#line 22 "D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\mbed\\targets\\hal\\rtl8711b\\spi_api.c"
...
static uint32_t ssi_interrupt(void *Adaptor)
{
	...
	uint32_t InterruptStatus = SSI_GetIsr(ssi_adapter->spi_dev);
	uint32_t DataFrameSize = SSI_GetDataFrameSize(ssi_adapter->spi_dev);

	SSI_SetIsrClean(ssi_adapter->spi_dev, InterruptStatus);

	if (InterruptStatus & (((uint32_t)0x00000001 << 1) | ((uint32_t)0x00000001 << 2) | ((uint32_t)0x00000001 << 3) | ((uint32_t)0x00000001 << 5))) {
		do { if ((LEVEL_INFO < LEVEL_NUMs) && (MODULE_SPI < MODULE_NUMs) && (ConfigDebug[LEVEL_INFO] & (1<<(MODULE_SPI)))) { _Pragma( "location=\".debug_trace\"") static const char traceFormat[] = "[INT] Tx/Rx Warning %x \n"; } }while(0);
	}

	if ((InterruptStatus & ((uint32_t)0x00000001 << 4)) ) {
		uint32_t TransLen = 0;
		...
		if (DataFrameSize > 8) {
			...
			ssi_adapter->RxData = (void*)(((uint16_t*)ssi_adapter->RxData) + TransLen);
		} else {
			
			ssi_adapter->RxData = (void*)(((uint8_t*)ssi_adapter->RxData) + TransLen);
		}
		
		if (ssi_adapter->RxLength == 0) {
			SSI_INTConfig(ssi_adapter->spi_dev, (((uint32_t)0x00000001 << 4) | ((uint32_t)0x00000001 << 3) | ((uint32_t)0x00000001 << 2)), 0);
			if (ssi_adapter->RxCompCallback != 0) {
				ssi_adapter->RxCompCallback(ssi_adapter->RxCompCbPara);
			}
		}
	}

	if (InterruptStatus & ((uint32_t)0x00000001)) {
		uint32_t TransLen = 0;
		...
		if (ssi_adapter->TxLength == 0) {
			SSI_INTConfig(ssi_adapter->spi_dev, (((uint32_t)0x00000001 << 1) | ((uint32_t)0x00000001)), 0);
			for (i=0;i<1000000;i++) {
			...
			if (ssi_adapter->TxData != 0) {
				if (ssi_adapter->TxIdleCallback != 0) {
		...
		if (DataFrameSize > 8) {
			...
			ssi_adapter->TxData = (void*)(((uint16_t*)ssi_adapter->TxData) + TransLen);
		} else {
			
			ssi_adapter->TxData = (void*)(((uint8_t*)ssi_adapter->TxData) + TransLen);
		}

		 
		if (ssi_adapter->TxLength == 0) {
			SSI_INTConfig(ssi_adapter->spi_dev, (((uint32_t)0x00000001 << 1)), 0);
			
			if (ssi_adapter->TxData != 0) {
				if (ssi_adapter->TxCompCallback != 0) {
...
static uint32_t ssi_int_read(void *Adapter, void *RxData, uint32_t Length)
{
	PHAL_SSI_ADAPTOR ssi_adapter = (PHAL_SSI_ADAPTOR) Adapter;
	uint32_t DataFrameSize = SSI_GetDataFrameSize(ssi_adapter->spi_dev);

	((Length != 0) ? (void)0 : io_assert_failed((uint8_t *)"D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\mbed\\targets\\hal\\rtl8711b\\spi_api.c", 194));

	 
	if (SSI_Busy(ssi_adapter->spi_dev)) {		
		do { if ((LEVEL_WARN < LEVEL_NUMs) && (MODULE_SPI < MODULE_NUMs) && (ConfigDebug[LEVEL_WARN] & (1<<(MODULE_SPI)))) { _Pragma( "location=\".debug_trace\"") static const char traceFormat[] = "ssi_int_read: SSI is busy\n"; } }while(0);
		return 0;
	}

	if (DataFrameSize > 8) {
		 
		ssi_adapter->RxLength = Length >> 1; 
	} else {
	...
	SSI_INTConfig(ssi_adapter->spi_dev, (((uint32_t)0x00000001 << 4) | ((uint32_t)0x00000001 << 3) | ((uint32_t)0x00000001 << 2)), 1);

	return (!0);
}


static uint32_t ssi_int_write(void *Adapter, uint8_t *pTxData, uint32_t Length)
{
	PHAL_SSI_ADAPTOR ssi_adapter = (PHAL_SSI_ADAPTOR) Adapter;
	uint32_t DataFrameSize = SSI_GetDataFrameSize(ssi_adapter->spi_dev);

	((Length != 0) ? (void)0 : io_assert_failed((uint8_t *)"D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\mbed\\targets\\hal\\rtl8711b\\spi_api.c", 223));
	
	if (DataFrameSize > 8) {
		 
		ssi_adapter->TxLength = Length >> 1; 
	} else {
	...
	SSI_INTConfig(ssi_adapter->spi_dev, (((uint32_t)0x00000001 << 1) | ((uint32_t)0x00000001)), 1);
...
static void ssi_dma_tx_irq(void *Data)
{
	...
	if (0 != ssi_adapter->TxCompCallback) {
		ssi_adapter->TxCompCallback(ssi_adapter->TxCompCbPara);
	}

	SSI_SetDmaEnable(ssi_adapter->spi_dev, 0, ((uint32_t)0x00000001 << 1));

	GDMA_ChnlFree(GDMA_InitStruct->GDMA_Index, GDMA_InitStruct->GDMA_ChNum);

	ssi_adapter->dma_en &= ~(1<<1);
}
...
static void ssi_dma_rx_irq(void *Data)
{
	...
	SSI_SetDmaEnable(ssi_adapter->spi_dev, 0, ((uint32_t)0x00000001));
	
	 
	if (0 != ssi_adapter->RxCompCallback) {
		ssi_adapter->RxCompCallback(ssi_adapter->RxCompCbPara);
	}

	GDMA_ChnlFree(GDMA_InitStruct->GDMA_Index, GDMA_InitStruct->GDMA_ChNum);

	ssi_adapter->dma_en &= ~(1<<0);
}

static uint32_t ssi_dma_send(void *Adapter, uint8_t *pTxData, uint32_t Length)
{
	PHAL_SSI_ADAPTOR ssi_adapter = (PHAL_SSI_ADAPTOR) Adapter;
	uint32_t ret = (!0);

	((Length != 0) ? (void)0 : io_assert_failed((uint8_t *)"D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\mbed\\targets\\hal\\rtl8711b\\spi_api.c", 292));
	((pTxData != 0) ? (void)0 : io_assert_failed((uint8_t *)"D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\mbed\\targets\\hal\\rtl8711b\\spi_api.c", 293));

	if (ssi_adapter->dma_en & (1<<1)) {
		return 0;
	}
	...
	SSI_SetDmaEnable(ssi_adapter->spi_dev, 1, ((uint32_t)0x00000001 << 1));
...
static uint32_t ssi_dma_recv(void *Adapter, uint8_t  *pRxData, uint32_t Length)
{
	PHAL_SSI_ADAPTOR ssi_adapter = (PHAL_SSI_ADAPTOR) Adapter;
	uint32_t ret = (!0);

	((Length != 0) ? (void)0 : io_assert_failed((uint8_t *)"D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\mbed\\targets\\hal\\rtl8711b\\spi_api.c", 317));
	((pRxData != 0) ? (void)0 : io_assert_failed((uint8_t *)"D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\mbed\\targets\\hal\\rtl8711b\\spi_api.c", 318));

	if (ssi_adapter->dma_en & (1<<0)) {
		return 0;
	}
	...
	SSI_SetDmaEnable(ssi_adapter->spi_dev, 1, ((uint32_t)0x00000001));

	return ret;
}

static uint32_t spi_stop_recv(spi_t *obj)
{
	...
	if (ssi_adapter->dma_en & (1<<0)) {
		...
		GDMA_ClearINT(GDMA_InitStruct->GDMA_Index, GDMA_InitStruct->GDMA_ChNum);
		GDMA_ChCleanAutoReload(GDMA_InitStruct->GDMA_Index, GDMA_InitStruct->GDMA_ChNum, ((uint32_t)0x00000003));
		GDMA_Cmd(GDMA_InitStruct->GDMA_Index, GDMA_InitStruct->GDMA_ChNum, 0);	
		DMAStopAddr = GDMA_GetDstAddr(GDMA_InitStruct->GDMA_Index, GDMA_InitStruct->GDMA_ChNum);
		GDMA_ChnlFree(GDMA_InitStruct->GDMA_Index, GDMA_InitStruct->GDMA_ChNum);

		ReceivedCnt = DMAStopAddr - (uint32_t)(ssi_adapter->RxData);
		ssi_adapter->RxLength-= ReceivedCnt;
		ssi_adapter->RxData = (uint8_t *)(ssi_adapter->RxData) + ReceivedCnt;
...
void spi_init (spi_t *obj, PinName mosi, PinName miso, PinName sclk, PinName ssel)
{
	...
	if ((obj->spi_idx != MBED_SPI0) && (obj->spi_idx != MBED_SPI1)) {
		do { if ((LEVEL_ERROR < LEVEL_NUMs) && (MODULE_SPI < MODULE_NUMs) && (ConfigDebug[LEVEL_ERROR] & (1<<(MODULE_SPI)))) { _Pragma( "location=\".debug_trace\"") static const char traceFormat[] = "spi_init: you should set spi_idx MBED_SPI0 or MBED_SPI1"; } }while(0);
		((0) ? (void)0 : io_assert_failed((uint8_t *)"D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\mbed\\targets\\hal\\rtl8711b\\spi_api.c", 401));
	}
	
	ssi_adapter->Index = spi_idx;
	obj->sclk = (uint8_t)sclk;

	if (spi_idx == 1) {
		ssi_adapter->Role = (1);
		RCC_PeriphClockCmd(((1<<(9)) | 2 << 30), ((1<<(18)) | (1<<29)), 1);
		Pinmux_Config(mosi, (2));
		Pinmux_Config(miso, (2));
		Pinmux_Config(sclk, (2));
		Pinmux_Config(ssel, (2));
	} else {
		ssi_adapter->Role = (0);
		RCC_PeriphClockCmd(((1<<(8)) | 2 << 30), ((1<<(16)) | (1<<29)), 1);
		Pinmux_Config(mosi, (3));
		Pinmux_Config(miso, (3));
		Pinmux_Config(sclk, (3));
		Pinmux_Config(ssel, (3));
	}

	
	SSI_StructInit(&SSI_InitStruct[spi_idx]);
	if (spi_idx == 1) {
		SSI_InitStruct[spi_idx].SPI_Role = (1);
	} else {
		PAD_PullCtrl(ssel, 0x02);
		PAD_PullCtrl(sclk, 0x02);
	}

	ssi_adapter->spi_dev = SPI_DEV_TABLE[ssi_adapter->Index].SPIx;
	ssi_adapter->IrqNum = SPI_DEV_TABLE[ssi_adapter->Index].IrqNum;
	VECTOR_IrqRegister((IRQ_FUN)ssi_interrupt, ssi_adapter->IrqNum, (uint32_t)ssi_adapter, 10);
	VECTOR_IrqEn(ssi_adapter->IrqNum, 10);
...
void spi_free (spi_t *obj)
{
	...
	VECTOR_IrqDis(ssi_adapter->IrqNum);
	VECTOR_IrqUnRegister(ssi_adapter->IrqNum);
...
void spi_format (spi_t *obj, int bits, int mode, int slave)
{
	...
	if (slave == 1) {
		if (spi_idx == 0) {
			...
			((0) ? (void)0 : io_assert_failed((uint8_t *)"D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\mbed\\targets\\hal\\rtl8711b\\spi_api.c", 562));
		}
	} else {
		if (spi_idx == 1) {
			ssi_adapter->Role = (1);
		} else {
			((0) ? (void)0 : io_assert_failed((uint8_t *)"D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\mbed\\targets\\hal\\rtl8711b\\spi_api.c", 568));
		}
	}
	...
	if (slave == 1) {
		if (SclkPolarity == (0)) {
			PAD_PullCtrl((uint32_t)obj->sclk, 0x01);
		}
		else {
			PAD_PullCtrl((uint32_t)obj->sclk, 0x02);
		}
	}
}
...
void spi_frequency (spi_t *obj, int hz)
{
	...
	SSI_SetBaud(ssi_adapter->spi_dev, (uint32_t)hz, CPU_ClkGet(0)/2);
}
...
void spi_slave_select(spi_t *obj, ChipSelect slaveindex)
{
	...
	if((ssi_adapter->Role == (1)) && (spi_idx == 1)){
		SSI_SetSlaveEnable(ssi_adapter->spi_dev,slaveindex);
	} else {
		((0) ? (void)0 : io_assert_failed((uint8_t *)"D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\mbed\\targets\\hal\\rtl8711b\\spi_api.c", 609));
	}
}
...
int32_t spi_slave_read_stream(spi_t *obj, char *rx_buffer, uint32_t length)
{
	...
	if (obj->state & (1<<1)) {
		do { if ((LEVEL_WARN < LEVEL_NUMs) && (MODULE_SPI < MODULE_NUMs) && (ConfigDebug[LEVEL_WARN] & (1<<(MODULE_SPI)))) { _Pragma( "location=\".debug_trace\"") static const char traceFormat[] = "spis int rx: state(0x%x) is not ready\r\n"; } }while(0);
	...
	obj->state |= (1<<1);
	if ((ret = ssi_int_read(ssi_adapter, rx_buffer, length)) != (!0)) {
		obj->state &= ~(1<<1);
	}

	return (ret == (!0)) ? HAL_OK : HAL_BUSY;
}
...
int32_t spi_slave_write_stream(spi_t *obj, char *tx_buffer, uint32_t length)
{
	...
	if (obj->state & (1<<2)) {
		do { if ((LEVEL_WARN < LEVEL_NUMs) && (MODULE_SPI < MODULE_NUMs) && (ConfigDebug[LEVEL_WARN] & (1<<(MODULE_SPI)))) { _Pragma( "location=\".debug_trace\"") static const char traceFormat[] = "spis int tx: state(0x%x) is not ready\r\n"; } }while(0);

		return HAL_BUSY;
	}

	obj->state |= (1<<2);
	if ((ret=ssi_int_write(ssi_adapter, (uint8_t *) tx_buffer, length)) != (!0)) {
		obj->state &= ~(1<<2);
	}
	return (ret == (!0)) ? HAL_OK : HAL_BUSY;
}
...
int32_t spi_master_read_stream(spi_t *obj, char *rx_buffer, uint32_t length)
{
	...
	if (obj->state & (1<<1)) {
		do { if ((LEVEL_WARN < LEVEL_NUMs) && (MODULE_SPI < MODULE_NUMs) && (ConfigDebug[LEVEL_WARN] & (1<<(MODULE_SPI)))) { _Pragma( "location=\".debug_trace\"") static const char traceFormat[] = "spim int rx: state(0x%x) is not ready\r\n"; } }while(0);
	...
	if ((ret = ssi_int_read(ssi_adapter, rx_buffer, length)) == (!0)) {
		...
		if ((ret=ssi_int_write(ssi_adapter, 0, length)) != (!0)) {
			obj->state &= ~(1<<1);
		}
	}
	else {
...
int32_t spi_master_write_stream(spi_t *obj, char *tx_buffer, uint32_t length)
{
	...
	if (obj->state & (1<<2)) {
		do { if ((LEVEL_WARN < LEVEL_NUMs) && (MODULE_SPI < MODULE_NUMs) && (ConfigDebug[LEVEL_WARN] & (1<<(MODULE_SPI)))) { _Pragma( "location=\".debug_trace\"") static const char traceFormat[] = "spim int tx: state(0x%x) is not ready\r\n"; } }while(0);
        ...
        char *rx_buffer, uint32_t length)
{
	...
	if (obj->state & ((1<<1)|(1<<2))) {
		do { if ((LEVEL_WARN < LEVEL_NUMs) && (MODULE_SPI < MODULE_NUMs) && (ConfigDebug[LEVEL_WARN] & (1<<(MODULE_SPI)))) { _Pragma( "location=\".debug_trace\"") static const char traceFormat[] = "spim int trx: state(0x%x) is not ready\r\n"; } }while(0);
...
int32_t spi_slave_read_stream_dma(spi_t *obj, char *rx_buffer, uint32_t length)
{
	...
	if (obj->state & (1<<1)) {
		do { if ((LEVEL_WARN < LEVEL_NUMs) && (MODULE_SPI < MODULE_NUMs) && (ConfigDebug[LEVEL_WARN] & (1<<(MODULE_SPI)))) { _Pragma( "location=\".debug_trace\"") static const char traceFormat[] = "spim dma rx: state(0x%x) is not ready\r\n"; } }while(0);

		return HAL_BUSY;
	}

	obj->state |= (1<<1);
	ret = ssi_dma_recv(ssi_adapter, (uint8_t *) rx_buffer, length);
	if (ret != (!0)) {
		obj->state &= ~(1<<1);
	}
	return (ret == (!0)) ? HAL_OK : HAL_BUSY;
...
int32_t spi_slave_write_stream_dma(spi_t *obj, char *tx_buffer, uint32_t length)
{
	...
	if (obj->state & (1<<2)) {
		do { if ((LEVEL_WARN < LEVEL_NUMs) && (MODULE_SPI < MODULE_NUMs) && (ConfigDebug[LEVEL_WARN] & (1<<(MODULE_SPI)))) { _Pragma( "location=\".debug_trace\"") static const char traceFormat[] = "spis dma tx: state(0x%x) is not ready\r\n"; } }while(0);

		return HAL_BUSY;
	}

	obj->state |= (1<<2);
	ret = ssi_dma_send(ssi_adapter, (uint8_t *) tx_buffer, length);
	if (ret != (!0)) {
		obj->state &= ~(1<<2);
	}
	return (ret == (!0)) ? HAL_OK : HAL_BUSY;
...
int32_t spi_master_read_stream_dma(spi_t *obj, char *rx_buffer, uint32_t length)
{
	...
	ret = ssi_dma_send(ssi_adapter, (uint8_t *) rx_buffer, length);
	if (ret != (!0)) {
		obj->state &= ~(1<<1);
	}
#line 985 "D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\mbed\\targets\\hal\\rtl8711b\\spi_api.c"
	return (ret == (!0)) ? HAL_OK : HAL_BUSY;
}
...
int32_t spi_master_write_stream_dma(spi_t *obj, char *tx_buffer, uint32_t length)
{
	...
	if (obj->state & (1<<2)) {
		do { if ((LEVEL_WARN < LEVEL_NUMs) && (MODULE_SPI < MODULE_NUMs) && (ConfigDebug[LEVEL_WARN] & (1<<(MODULE_SPI)))) { _Pragma( "location=\".debug_trace\"") static const char traceFormat[] = "spim dma tx: state(0x%x) is not ready\r\n"; } }while(0);
	...
	char *rx_buffer, uint32_t length)
{
	...
	if (obj->state & ((1<<1)|(1<<2))) {
		do { if ((LEVEL_WARN < LEVEL_NUMs) && (MODULE_SPI < MODULE_NUMs) && (ConfigDebug[LEVEL_WARN] & (1<<(MODULE_SPI)))) { _Pragma( "location=\".debug_trace\"") static const char traceFormat[] = "spi_master_write_and_read_stream: state(0x%x) is not ready\r\n"; } }while(0);
		return HAL_BUSY;
	}
...
int32_t spi_slave_read_stream_dma_timeout(spi_t *obj, char *rx_buffer, uint32_t length, uint32_t timeout_ms)
{
	...
	((timeout_ms > 0) ? (void)0 : io_assert_failed((uint8_t *)"D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\mbed\\targets\\hal\\rtl8711b\\spi_api.c", 1068));
	
	if (obj->state & (1<<1)) {
		do { if ((LEVEL_WARN < LEVEL_NUMs) && (MODULE_SPI < MODULE_NUMs) && (ConfigDebug[LEVEL_WARN] & (1<<(MODULE_SPI)))) { _Pragma( "location=\".debug_trace\"") static const char traceFormat[] = "spi_slave_read_stream_dma: state(0x%x) is not ready\r\n"; } }while(0);
		return -HAL_BUSY;
	}
	...
	while (1) {
		...
		if (SYSTIMER_GetPassTime(StartCount) > timeout_ms) {
			...
			do { if ((LEVEL_INFO < LEVEL_NUMs) && (MODULE_SPI < MODULE_NUMs) && (ConfigDebug[LEVEL_INFO] & (1<<(MODULE_SPI)))) { _Pragma( "location=\".debug_trace\"") static const char traceFormat[] = "Slave is timeout\n"; } }while(0);
			break;
		}
	}
...
int32_t spi_slave_read_stream_timeout(spi_t *obj, char *rx_buffer, uint32_t length, uint32_t timeout_ms)
{
	...
	((timeout_ms > 0) ? (void)0 : io_assert_failed((uint8_t *)"D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\mbed\\targets\\hal\\rtl8711b\\spi_api.c", 1189));
	
	if (obj->state & (1<<1)) {
		do { if ((LEVEL_WARN < LEVEL_NUMs) && (MODULE_SPI < MODULE_NUMs) && (ConfigDebug[LEVEL_WARN] & (1<<(MODULE_SPI)))) { _Pragma( "location=\".debug_trace\"") static const char traceFormat[] = "spi_slave_read_stream: state(0x%x) is not ready\r\n"; } }while(0);
		return -HAL_BUSY;
	}
...
void spi_disable(spi_t *obj)
{
	...
	if (spi_idx == 1) {
		RCC_PeriphClockCmd(((1<<(9)) | 2 << 30), ((1<<(18)) | (1<<29)), 0);
	} else {
		RCC_PeriphClockCmd(((1<<(8)) | 2 << 30), ((1<<(16)) | (1<<29)), 0);
	}
}
