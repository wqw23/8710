#line 1 "D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\mbed\\targets\\hal\\rtl8711b\\analogin_api.c"
...
#line 19 "D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\mbed\\targets\\hal\\rtl8711b\\analogin_api.c"
...
#line 28 "D:\\vm-share\\506\\LeIotProjLib\\..\\rtk8710_sdk\\component\\common\\mbed\\hal\\analogin_api.h"
...
#line 21 "D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\mbed\\targets\\hal\\rtl8711b\\analogin_api.c"
...
#line 23 "D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\mbed\\targets\\hal\\rtl8711b\\analogin_api.c"
...
void ADC_ReceiveBuf_ACUT(uint32_t *pBuf)
{
	uint32_t isr = ADC_GetISR();
	ADC_TypeDef *adc = ((ADC_TypeDef *) 0x40010000);
	uint32_t	AdcTempDat;
	
	 
	ADC_INTClear();
	ADC_INTConfig(((uint32_t)0x00000001 << 4)|((uint32_t)0x00000001 << 5), 1);
	...
	while (1) {
		isr = ADC_GetISR();
		if (isr & (((uint32_t)0x00000001 << 4) | ((uint32_t)0x00000001 << 5))) {
			*pBuf = (uint32_t)ADC_Read();
			*(pBuf+1)= (uint32_t)ADC_Read();

			ADC_INTClear();
			ADC_INTConfig(((uint32_t)0x00000001 << 4)|((uint32_t)0x00000001 << 5), 0);
...
void analogin_init(analogin_t *obj, PinName pin)
{
	...
	ConfigDebugErr &= (~(0x00040000|0x00000002));
	ConfigDebugInfo&= (~(0x00040000|0x00000002));

	adc_idx = pinmap_peripheral(pin, PinMap_ADC);
	do { if ((ConfigDebugErr & 0x40000000)) DiagPrintf("\r" "analogin_init [%x:%x ]\n", pin, adc_idx);}while(0);
	((adc_idx != NC) ? (void)0 : io_assert_failed((uint8_t *)"D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\mbed\\targets\\hal\\rtl8711b\\analogin_api.c", 105));
	...
	VECTOR_IrqDis(ADC_IRQ);

	 
	PLL2_Set((0x00000001 << 25), 1);

	 
	RCC_PeriphClockCmd(((1<<(0)) | 3 << 30), ((1<<(24)) | (2<<29)), 1);
...
float analogin_read(analogin_t *obj)
{
	...
	AnaloginDatMsk = (uint32_t)(AnaloginDatMsk<<((uint32_t)(16*(AnaloginIdx&0x01))));
	AnalogDat = AnaloginTmp[(AnaloginIdx/2)];
	AnalogDat = (AnalogDat & AnaloginDatMsk);    
	AnalogDat = (AnalogDat>>((uint32_t)(16*(AnaloginIdx&0x01))));
...
void  analogin_deinit(analogin_t *obj)
{
	...
	PLL2_Set((0x00000001 << 25), 0);

	 
	RCC_PeriphClockCmd(((1<<(0)) | 3 << 30), ((1<<(24)) | (2<<29)), 0);
}
