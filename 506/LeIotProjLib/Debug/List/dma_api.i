#line 1 "D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\mbed\\targets\\hal\\rtl8711b\\dma_api.c"
...
#line 1 "D:\\vm-share\\506\\LeIotProjLib\\..\\rtk8710_sdk\\component\\common\\mbed\\hal_ext\\dma_api.h"
...
#line 29 "D:\\vm-share\\506\\LeIotProjLib\\..\\rtk8710_sdk\\component\\common\\mbed\\hal_ext\\dma_api.h"
...
#line 91 "D:\\vm-share\\506\\LeIotProjLib\\..\\rtk8710_sdk\\component\\common\\mbed\\hal_ext\\dma_api.h"
...
#line 18 "D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\mbed\\targets\\hal\\rtl8711b\\dma_api.c"
...
static void dma_memcpy_int(void *pData)
{
	...
	GDMA_ChCleanAutoReload(dma_obj->index, dma_obj->ch_num, ((uint32_t)0x00000002));

	 
	GDMA_ClearINT(dma_obj->index, dma_obj->ch_num);

	GDMA_Cmd(dma_obj->index, dma_obj->ch_num, 0);

	if (dma_obj->user_cb != 0) {
...
void dma_memcpy_init(gdma_t *dma_obj, dma_irq_handler handler, uint32_t id)
{
	...
	dma_obj->index = 1;
	ch_num = GDMA_ChnlAlloc(dma_obj->index, (IRQ_FUN) dma_memcpy_int, (uint32_t)dma_obj, 10);
	if (0xFF == ch_num) {        
		do { if ((ConfigDebugErr & 0x40000000)) DiagPrintf("\r" "%s: Cannot allocate a GDMA Channel\n", __FUNCTION__);}while(0);
		return;
	}
...
void dma_memcpy(gdma_t *dma_obj, void *dst, void* src, uint32_t len)
{
	...
	GDMA_InitStruct.GDMA_IsrType = (((uint32_t)0x00000001)|((uint32_t)0x000000010));

	do { if ((LEVEL_INFO < LEVEL_NUMs) && (MODULE_GDMA < MODULE_NUMs) && (ConfigDebug[LEVEL_INFO] & (1<<(MODULE_GDMA)))) { _Pragma( "location=\".debug_trace\"") static const char traceFormat[] = "%s: ==> Src=0x%x Dst=0x%x Len=%d\r\n"; } }while(0);
	
	if ((((uint32_t)src & 0x03)==0) &&	(((uint32_t)dst & 0x03)==0) && ((len & 0x03)== 0)) {
		 
		GDMA_InitStruct.GDMA_SrcMsize   = ((uint32_t)0x00000002);
		GDMA_InitStruct.GDMA_SrcDataWidth = ((uint32_t)0x00000002);
		GDMA_InitStruct.GDMA_DstMsize = ((uint32_t)0x00000002);
		GDMA_InitStruct.GDMA_DstDataWidth = ((uint32_t)0x00000002);
		GDMA_InitStruct.GDMA_BlockSize = len >> 2;
	} else {
		GDMA_InitStruct.GDMA_SrcMsize   = ((uint32_t)0x00000002);
		GDMA_InitStruct.GDMA_SrcDataWidth = ((uint32_t)0x00000000);
		GDMA_InitStruct.GDMA_DstMsize = ((uint32_t)0x00000002);
		GDMA_InitStruct.GDMA_DstDataWidth = ((uint32_t)0x00000000);
		GDMA_InitStruct.GDMA_BlockSize = len;
	}

	GDMA_InitStruct.GDMA_SrcAddr = (uint32_t)src;
	GDMA_InitStruct.GDMA_DstAddr = (uint32_t)dst;                

	GDMA_Init(dma_obj->index, dma_obj->ch_num, &GDMA_InitStruct);
	GDMA_Cmd(dma_obj->index, dma_obj->ch_num, 1);
}
