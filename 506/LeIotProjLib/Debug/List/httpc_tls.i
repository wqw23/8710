...
void *httpc_tls_new(int *sock, char *client_cert, char *client_key, char *ca_certs)
{
	...
	int ret = 0;
	struct httpc_tls *tls = 0;

	memory_set_own(pvPortMalloc, vPortFree);
	tls = (struct httpc_tls *) pvPortMalloc(sizeof(struct httpc_tls));

	if(tls) {
		ssl_context *ssl = &tls->ctx;

		_memset(tls, 0, sizeof(struct httpc_tls));
		...
		if(client_cert && client_key) {
			if((ret = x509_crt_parse(&tls->cert, (const unsigned char *) client_cert, prvStrLen((const unsigned char *) client_cert))) != 0) {
				rtl_printf("\n[HTTPC] ERROR: x509_crt_parse %d\n", ret);
				ret = -1;
				goto exit;
			}

			if((ret = pk_parse_key(&tls->key, (const unsigned char *) client_key, prvStrLen((const unsigned char *) client_key), 0, 0)) != 0) {
		...
		if(ca_certs) {
			...
			if((ret = x509_crt_parse(&tls->ca, (const unsigned char *) ca_certs, prvStrLen((const unsigned char *) ca_certs))) != 0) {
				rtl_printf("\n[HTTPC] ERROR: x509_crt_parse %d\n", ret);
				ret = -1;
				goto exit;
			}

			ssl_set_ca_chain(ssl, &tls->ca, 0, 0);
...
exit:
	...
	return (void *) tls;
#line 243 "D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\network\\httpc\\httpc_tls.c"
}
...
void httpc_tls_free(void *tls_in)
{
	...
	pk_free(&tls->key);
	vPortFree(tls);
#line 263 "D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\network\\httpc\\httpc_tls.c"
}
...
int httpc_tls_handshake(void *tls_in, char *host)
{
	...
	return ret;
#line 298 "D:\\vm-share\\506\\rtk8710_sdk\\component\\common\\network\\httpc\\httpc_tls.c"
}
